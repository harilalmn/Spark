...
**Summary:** 19 done · 8 in progress · 201 open · 4 withdrawn
...
nineteen ADRs are written and indexed, and the lacing specification exists.
...
| E1-T23 | Check in public-API baselines for `Spark.Api` and `Spark.Geometry` | Open | `Microsoft.CodeAnalysis.PublicApiAnalyzers [5.6.0]` is pinned but unreferenced. Makes every change to the public surface a reviewed line in a text file, which is half the mechanism behind [E11-T14](#e11--quality-and-verification). Kept as a **review aid, not a compatibility guarantee** — nothing is published, so nobody's restore breaks; what the baseline prevents is a public surface growing by accident (ADR-0019) |
