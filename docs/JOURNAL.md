...
**Last updated:** 2026-09-13 (`E2-T46`: §3.8 infrastructure, in progress)
**Protocol version:** 2
...
| **Next action** | **Assess §3.8's 33 unassessed members against the assembly and rewrite the section with a measured *Reachable* column**, the shape `E2-T42` … `E2-T45` established. **Spark has no `Geometry` base class and will not acquire one** — ADR-0002's value types are `readonly struct`s and a common base would box every one — so every row names the member on the type that actually carries it, and the section has to say that once rather than 29 times. Three families: **transformation (12)**, which is `Transform` applied through `TransformedBy`/`Of*` and whose precedent `E2-T45` set on real rows yesterday; **measurement and intersection (10)**, five of them §6.1's exact-boolean count; and **`ToJson`/`FromJson`**, which are `GeometryJson.Serialize`/`Deserialize` — already proved, because `E2-T45` un-excused that type to name them. **Read the assembly, never the document** ([N160](NOTES.md)): four assessments in four days each found the register claiming *absent* about something that shipped weeks ago. Watch for `IsAlmostEqualTo` (`Tolerance.AreEqual`?), `Approximate()`, `ContextCoordinateSystem` and `OrientedBoundingBox`, which §3.1 already flags as an open question. |
| **Verify with** | For `E2-T46`: the four parity checks green with the residue budget **exact**; §3.8's per-type counts still agreeing with the manifest and a *Reachable* column measured from it; §5's refusal count re-derived if any of the 56 changes; the TASKS row closed with the transformation/measurement split either confirmed or corrected **in writing**; `docs/progress.html` regenerated by `scripts/build-progress.py`; and the gates — build clean with zero warnings, **3319** tests over ten executables with zero failures and zero skips, format clean. |
