...
**Last updated:** 2026-09-13 (`E2-T46`: §3.8 assessed, and a solid that cannot be moved)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps.** Two assessments landed today and the second found a real hole rather than a bookkeeping one: **a `Brep` cannot be transformed** — no member, no kernel operation, no node — so a solid can be unioned, filleted, hollowed and exported and cannot be *moved* (`E2-T70`, [N161](NOTES.md)). **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
