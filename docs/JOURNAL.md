...
**Last updated:** 2026-09-13 (`E2-T71` family (2) closes: planarity, and the plane a curve lies in)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps.** Eleven steps landed today. The last four all *build* what the register found rather than measuring it, and `E2-T71` is three families down of five. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
| **Last completed step** | **`E2-T71` family (2) — `Curve.IsPlanar` and `Curve.PlaneOf`, and it answered five rows rather than the two it is named for.** **The name is not Dynamo's, and that is the decision in the step.** Dynamo has a `Curve.Normal` property; Spark already has `Curve.NormalAt(parameter)`, which is the **Frenet** normal at a point and a completely different quantity — so importing Dynamo's name would have put two unrelated things under almost one name, which is the sort of collision a parity register exists to *catch* rather than to propagate. The member is **`PlaneOf(in Tolerance)`** returning `Plane?`, Dynamo's `Normal` is `PlaneOf()?.Normal`, and returning the whole frame answers `PolyCurve.BasePlane` with the same member. **The pair answers two different questions on purpose, and a straight line is why**: *is there a plane containing this curve* and *which plane is it* are not the same question, so a `Line` is `IsPlanar` **true** and `PlaneOf` **null**. `null` therefore has two causes — no plane, or infinitely many — and `IsPlanar` is what separates them. **The fit is not the test, and that is the whole implementation**: `Plane.FromBestFit` returns a plane for *any* points at all, so returning it unchecked makes a helix planar; the residual against the caller's tolerance is what decides. Proved by deleting the residual check and watching three tests go red, **including the one that matters** — a curve **nearly** planar and not, because a wildly non-planar curve fails even a broken implementation by accident. **Two rows closed as compositions on it**: `Extrude(double)` and `ExtrudeAsSolid(double)` extrude along the curve's own normal and were waiting on nothing else. **One name I invented and had to correct before it was committed**: the first draft of those two rows named `CurveOffset.Extrude`, which does not exist — the members are `ExtrusionSurface` and `IBrepKernel.Extrude`, and [N160](NOTES.md)'s rule is exactly this, that a register must read the assembly every time. Residue **292 → 300**, four types times two members. 13 tests, **3381 → 3394**. **Before it:** **`E2-T71` step B — `Arc`, `Circle` and `EllipseCurve` convert exactly, and the quarter-circle has one copy.** **The work was the lifting, not the mathematics.** `SurfaceConversion.Arc` has produced an exact rational quadratic for a circular sweep since `E2-T19` — control points at the **tangent intersection**, radius `r / cos(θ/2)` and not the arc's midpoint, weights `cos(θ/2)` and not `cos θ`, split at every half turn because the middle weight reaches zero there — and it was **private to that one class**. It is now an internal `RationalArcs` both layers call. **The shared copy proved itself inside the same step**: replacing `cos(θ/2)` with `cos θ` reddens the new curve tests **and eight surface tests**, which is precisely what deriving it twice would have stopped happening. **The ellipse cost nothing beyond the circle** — an ellipse is an affine image of a circle and an affine map carries a rational B-spline to a rational B-spline, so it is the circle's points, weights and knots mapped through a different frame, with no second construction to get wrong. **`Arc` and `EllipseCurve` needed their knots slid back by the start angle**, because both are parameterised from their own start rather than from the plane's x axis. **Two tests were wrong and the code was right, and one of them was worth the trip**: the parameterisation test sampled an *eighth* of the way round a circle — which is the **middle of the first of four spans**, and a span's midpoint maps to its arc's midpoint by symmetry, so it is one of the nine parameters where the two parameterisations *do* agree. The corrected test checks a tenth, and now pins the agreement at the nine as a fact about the construction. Residue **289 → 292**: three overrides, and `RationalArcs` adds nothing because it is internal. 5 tests, **3376 → 3381**. **Before it:** **`E2-T71` step A — `Curve.ToNurbsCurve`, and a result that says whether the conversion was exact.** The finding of `E2-T41` step A, built the day after it was filed. **The design is [N165](NOTES.md)'s doing, and it is the whole of the row**: seven of Spark's eight curve types convert exactly and `Helix` *provably never can*, so the member returns a **`NurbsConversion`** — the curve, and whether it is the same curve — rather than a bare `NurbsCurve` that a caller would have no way to interrogate. That is `CurveIntersections`' shape, where the caveat travels with the answer instead of being something you have to know to ask for. **Exact in step A**: `Line` and `PolyLine` at degree 1, and **those two preserve the parameterisation as well as the point set** — `PointAt(t)` gives the same point on both, which nothing above degree 1 does and which is pinned rather than assumed; and `NurbsCurve`, which returns itself and **needed** the override, because the fallback would otherwise have sampled and interpolated the very thing being asked for and called the result approximate. **An exact conversion is asserted *as exact***, bit for bit at every sampled parameter, because a tolerance-sized assertion passes just as happily for a conversion that is merely very good — which is [N165](NOTES.md)'s lesson pointed the other way. **`ModelWriter` stopped guessing, and the code it replaced was wrong in both directions**: `Approximated` was set from the fact that its `switch` had fallen through to `default`, so an exactly-convertible curve with no case above would have been exported perfectly and reported as approximate, and a case added above would have been reported as exact whether it was or not. Its sampling was a flat 64 points — on a three-turn helix, a chord every 16° and a deviation of about **two per cent of the radius**, measured nowhere — and is now a sag from the curve's own length through `Tolerance.ForScale`. The branch proved red is **`IsExact` itself**: flipping the fallback to report `true` fails three tests, without which the flag is decoration. Residue **284 → 289**, and `NurbsConversion` is **counted rather than excused** where `CurveIntersections` is excused — the precedent was split and the later choice is followed, because a counted member is visible in that number and an excused one is invisible for ever. 8 tests, **3368 → 3376**. **Before it:** **`E2-T73` — `Helix`, the eighth curve type, closed the same day it was filed.** **The first row of this whole run that builds what the register found instead of measuring it**: seven parity rows move `Planned` → `Done`, §3.2 goes 111 → **118 of 187** and the headline 392 → **399 of 545**, and every earlier move of that number came from an assessment finding Spark further ahead than its own prose claimed. **It cost what `D27` said it would, because a helix travels at a constant speed** — `LengthAt`, `ParameterAtLength`, `PointAtLength`, `DivideEqually` and `DivideByLength` are closed form rather than iterative, where the ellipse needed Gauss–Legendre and Newton for the same five. **Three conventions, every one borrowed from this layer rather than invented for it**: a negative sweep flips the axis so the domain stays increasing (`Arc`'s, and the whole of `Curve` relies on it); `AxisPoint` reports the point on the axis *level with the start* rather than the origin the caller passed, because only the axis **line** is in the curve (`Arc.StartAngle`'s); and a **zero pitch is refused with a message that names `Arc`** (`Circle.Trimmed`'s — a circle that is not closed is not a circle). **A negative pitch is not refused: that is handedness**, and unlike a negative sweep it cannot be reached by flipping anything. **Two branches were proved red by removing them**, and the second is the one that matters: **the axial factor in `TransformedBy`**. `CircularArcs.TransformFrame` returns the **radial** factor, the two are equal under a uniform scale, and taking one for the other gives a helix of exactly the right shape and the wrong height — which passes every test that does not stretch along the axis. **The arc length is never pinned against the expression the type computes**: the anchor is a summed chord of 400,000 samples of `PointAt`, which contains no closed form at all, and it agrees to nine figures from below. **And the step found a live defect nothing in the suite could see** ([N164](NOTES.md)): `ReferenceCatalog`'s pin list against `CS0104` is written by hand and said *the nine that collide*; there were ten, so `Helix` was ambiguous in **every code block in the application** while 3,368 tests passed. The **help-sample compiler** caught it — three documents away, and only because the topic happened to name the type. A façade nobody documented would have shipped broken. That set is now derived by reflection. Residue **277 → 284**, seven, which is exactly what `Circle` and `Arc` each contribute: the six named members come off, `ToString` is excused, and the seven `Curve` overrides are counted. 26 tests, **3340 → 3368**. **Before it:** **`E2-T70` step B — a curve can be cut by a surface, and the row closes.** `Curve.IntersectWith(Surface, in Tolerance)`, which had **no member anywhere** and is the half of the queries-take-only-a-point finding a managed implementation should own. **A surface has no signed distance of its own**, so one is built at each of 128 samples — `Surface.ClosestPoint` for the nearest point, dotted with the normal there — and a change of sign brackets a crossing; Newton on `C(t) - S(u, v) = 0` refines it, three equations in three unknowns, landing on curve and surface at once so the result can carry **both** parameters. **A singular Jacobian is a tangency, not a failure**, and bisection answers it. **Anchored on a closed form the kernel does not contain** — a line against a plane, a hundred random cases, matched to a **nanometre**, a thousand times tighter than the tolerance given — and **that assertion caught a design error before it shipped**: the refinement was stopping as soon as the residual was inside the tolerance, and **the tolerance decides what counts as *meeting*, not how accurately the answer is *reported*.** **One test was wrong and the code was right** — a circle across a cylinder crosses four times, not two. Twelve tests, the two that matter **proved to go red**. Residue **270 → 277**, a rise from new code and the first since nothing was left `Unassessed` to hide one. **Surface/surface stays behind the seam** (ADR-0002), so `Geometry.Intersect` stays `Planned`. **Before it:** **`E2-T40` — §3.1's value layer re-measured, and the register is finished.** 78 rows, and **§3.1 is 113 of 133, not the 95 the task row claimed**. With it **every one of the 837 manifest rows carries a status and a written reason**: `Done` 392, `Planned` 124, `Not planned` 123, `Needs a decision` 198, **`Unassessed` 0**. The document's headline moves from *99 of 575* to **392 of 545 — and it is now taken from the manifest rather than checked against it**. **This was the section that was supposed to show over-claims**, because it is the only one whose figure was a hand count rather than a zero, and the journal said so before the work started. It under-claimed by 14, **for the seventh section running** — written up as [N163](NOTES.md), which is about why a register's error has a direction and why only the reverse check can catch it. Six of the 14 are `CoordinateSystem`'s transformation family, where the section argued the `Transform` split at length and then counted it as a gap; six are `BoundingBox`, reachable the moment there was geometry to bound. **`Q17` filed** — does Spark want an `OrientedBox` — which had sat in prose as *recorded as an open question* for a year, and is the only question left in §3.1 and §3.8 outside the T-Splines. **Residue 303 → 270, the largest fall it has made, with no type un-excused** — and it now measures what it says, because nothing is left for a future assessment to name. **Before it:** **`E2-T41` step B — §3.2's ten concrete curve types, and the row closes at 111 of 187.** 105 rows: `Line` 5 of 6, `Arc` 8 of 14, `Circle` 7 of 8, **`Ellipse` 8 of 8, `EllipseArc` 9 of 9 and `Rectangle` 8 of 8 — complete**, `Helix` 0 of 7, `NurbsCurve` 11 of 15, `PolyCurve` 7 of 21, `Polygon` 8 of 9. `EllipseCurve` answers two Dynamo types on its own. **Two more decisions made rather than deferred**: **`Helix` goes in** (`D27`, `E2-T73`) — the cheapest type the hierarchy can gain, because a helix travels at a constant speed and every arc-length member is closed form for it — and **`Rectangle` and `Polygon` stay `PolyLine` factories**, recorded as built, with the price visible: `Rectangle.Width` is `SegmentAt(0).Length`. **Three findings, each one missing idea rather than a list** (`E2-T72`): **no curve fitting of any kind**, though `Plane.FromBestFit` and `NurbsCurve.FitPoints` exist; **one fillet, and it takes two lines**, where four rows want the generalisation; and **no periodic NURBS curve**, which is [N156](NOTES.md)'s trap a third time. **Residue 318 → 303, the largest fall it has made — and it falsified [N162](NOTES.md) in the step after the note was written.** The note predicted a section of constructions would cost; it saved, because the curve types were already in the register. The note carries its own correction rather than a quiet edit. **Before it:** **`E2-T41` step A — §3.2's 82 `Curve` rows assessed, 40 reachable.** The largest single type in the inventory, from **five `Done` and 77 never looked at** to **40 `Done`, 20 `Planned`, 14 `Needs a decision`, 8 `Not planned`**. The contract this section was written to shape now answers half the type it was written against — the sixth pass in a row to find Spark ahead of the register. **Three calls made rather than deferred.** §5 [i]'s duplicated trim family **has a survivor at last** — the `Parameter*` set — so its eight refusals are counted where §5 had carried them as uncounted since the document was written. §6.3's four-parameterisations question is **half answered**: the `AtDistance` family *is* arc length and is `Done`; what `AtSegmentLength` measures is still undeducible, so six members stay `Needs a decision` rather than becoming guesses. And `ApproximateWithArcAndLineSegments` is **deliberately not refused** under §5 [j] though its signature qualifies — [j] refuses a *form* where the capability has a tolerance-taking home in Spark, and a biarc approximation has none. **Nine of the trim family are `Done` and seven by composition**, because `Curve.Trimmed` twice is a split. **The finding is `ToNurbsCurve`: Spark has no curve-to-NURBS conversion anywhere**, while `SurfaceConversion` converts five of eight analytic surfaces exactly. Twenty gaps filed as `E2-T71`. **Residue 321 → 318, the first fall since `E2-T42` step B** — 13 members named against 10 dragged in by claiming `CurveOffset` and `ExtrusionSurface` — and what that direction means is [N162](NOTES.md). **Before it:** **`E2-T70` step A — a solid can be moved, and a mirror no longer turns one inside out.** `Brep.TransformedBy`, with `Solid.Translate`, `Rotate` and `Mirror` as nodes and `MeshFace.Reversed()` beside them. **Taken ahead of the queue on purpose** — a defect found today outranks the sixth audit pass. Geometry moves, **every index is left alone**, it needs **no provider**, and a resident shape **materialises**, which `BrepResidency.Materialise` already named as a structural demand (asking the provider instead is not available: no such operation, and `E13-T21` blocks adding one). **Every face flips when `Determinant < 0`**, or a mirrored solid is inside out with every index still in range and `Validate()` still empty. **Asking whether `Mesh.TransformedBy` did the same found a live bug**: it had been leaving every face wound backwards under a mirror since M5. Nine tests, and the two that matter were **proved to go red** with the branch removed. Residue **319 → 321**, the first rise from new code rather than from an assessment. **Before it:** **`E2-T46` — §3.8's 89 infrastructure rows assessed, 24 reachable.** 56 were already refused under §5; the 33 that were open are `Geometry`'s 29 and `DesignScriptEntity`'s four. The transformation family is **14, not the 12 the section claimed**, and all 14 are `Done`. **The finding: `Brep` has no transform at all** — `TransformedBy` is on eleven other types, `IBrepKernel` has no transform operation, there is no node, and nothing has reported it since M6 (`E2-T70`). **The second: every query takes a *point*, never another geometry**, so `ClosestPointTo`, `DistanceTo`, `DoesIntersect` and `Intersect` are one gap under four names, with curve/surface intersection missing entirely. `IsAlmostEqualTo`, `Approximate` and `ContextCoordinateSystem` refused **in that form** as the new §5 [j]. `DesignScriptEntity.Tessellate` had landed and was listed as planned — the fifth under-claim in five assessments. Residue **292 → 319**, the largest move it has made, and the price of claiming `Transform` rather than drift. **Before it:** **`E2-T45` — §3.6's 65 mesh rows assessed, 41 reachable.** From *0 reachable*, and almost none of it new code. `IndexGroup` **is** `MeshFace`, 10 of 10. The eight-missing-repair-members finding is **confirmed with one correction**: `CloseCracks()` is already `Mesh.Welded(tolerance)`, so six are missing (`E2-T68`) and `MakeHollow` is a decision (`Q16`); `MakeWatertight` is *not* `Welded`, and saying it was would have been [N156](NOTES.md)'s trap a fourth time. Two harness corrections: **`Spark.Geometry.Io` joined the delivering assemblies** — STL, PLY, OBJ and glTF read as absent — and **`GeometryJson` stopped being excused** on a §5 [g] refusal §5 [g] does not make. Residue **285 → 292**, a rise that is the register working, with its cause written down. Five stale numbers corrected, and the residue check's own instruction stopped being truncated by `Assert.Empty`. **Before it:** **`Chamfer` gets a test, and `E13-T22` is found and pinned.** The one kernel operation nothing called now has two tests, one against a closed form; and `Solid.HollowOpen` was written, tested, found broken and **deleted in the same step** because managed face indices are not OCCT's ([N159](NOTES.md)). Nothing shipped is wrong — every caller passes the whole set, and a permutation of a whole set is that set. **Before it:** **`E2-T43` — §3.4's 55 solid rows assessed, 24 reachable.** All 16 of `Solid`'s reachable operations name `Spark.Api.IBrepKernel`; `Volume` and `Area` are `Done` on the *tessellation* and `Centroid` is absent (`E2-T67`); `BrepPrimitives` has **no sphere and no cone** (`E2-T66`); and the 14 parameter-recovery refusals line up with §5 exactly. **Before it:** **`E11-T30` — the register reads what delivers the geometry.** `Spark.Geometry`, `Spark.Api` and `Spark.Nodes.Core` for the rename-catcher; `Spark.Geometry` alone for the reverse direction, and the residue budget unchanged at 285 is the proof the two stayed apart. `Surface.Repair()` went from `Not planned` to `IBrepKernel.Heal` — a true premise that had reached a false conclusion ([N158](NOTES.md)). §3.3 at 48 of 106. **Before it:** **`E2-T42` step B — `NurbsSurface` and `PolySurface`, and the row closes.** 12 of 17 and 6 of 18. `IsPeriodicInU` / `InV` refused the easy match with `IsClosedU` / `InV`, which is [N156](NOTES.md)'s trap a third time; `PolySurface.BySolid` is `Done` because Spark needs no member for it; `UnconnectedBoundaries()` became reachable the day it was assessed, through `E2-T65`. **Before it:** **`E2-T42` step A — `Surface`'s 46 rows assessed.** The evaluation family is **complete**, not missing its curvature half as the row claimed; twelve rows say `Planned` for capabilities Spark delivers behind `Spark.Api.IBrepKernel`, which is the register reading one assembly (`E11-T30`, [N158](NOTES.md)); and naming a bare type silently escaped the wholesale-excuse rule until this step found it. **Before it:** **`E2-T65` — the topology read backwards, without a back-pointer.** `BrepAdjacency` computes the reverse in one pass and the **caller** keeps it, so the model still holds each fact once; four of the twelve members needed no index at all. `E2-T44` closes with §3.5 at 31 of 33. **Before it:** **`E2-T44` step A — the 33 topology rows assessed.** 19 reachable and 14 not, §3.5 rewritten with a measured *Reachable* column, and the 12-member gap filed as `E2-T65`: the model stores each relationship once, in one direction. Two more are `E2-T64`'s pcurves, and `Face.SurfaceGeometry()` is trimmed where `BrepFaceView.Surface` is not — [N156](NOTES.md)'s difference caught a second time. **Before it:** **`E11-T23` step B — the reverse direction, and the 89 name matches reviewed.** An exclusions file that excuses **by rule and never member by member** (47 rules over 877 public members), a residue budget checked for **exact** equality rather than as a ceiling, and one row in 89 demoted because the name hid a real difference ([N156](NOTES.md)). **Before it:** **The client's decisions of 2026-09-12.** Open source at release, no certificate, Actions stopped, STEP verified in AutoCAD, and our own kernel declined. **Before it:** **`E3-T19` — closed on the evidence.** Five additive format versions, each file written at the lowest that can read it; no migration has had a caller. **Before it:** **`E11-T23` step A — the Dynamo parity manifest.** 837 rows from ProtoGeometry's metadata, calibrated against the coverage document's counts; well-formed, totals and rename-catcher checks. **Before it:** **`E3-T20` step D — `--open` takes a bundle too.** The startup door unpacks a `.sparkz` as File ▸ Open does. **Before it:** **`E3-T20` step C — the application opens a bundle and shares one.** File ▸ Open takes a `.sparkz` and opens its graph as a file; File ▸ Share as bundle… packs the saved graph beside it. **Before it:** **`E3-T20` step B — `spark pack`, and a bundle wherever a graph goes on the command line.** Opened into a temporary folder that is removed afterwards; its packages gated as any folder beside a graph. **Before it:** **`E3-T20` step A — the `.sparkz` bundle packs and opens.** A manifest, the graph and its package folder; five refusals, nothing written on any of them. **Before it:** **`E3-T14` step B — the evaluating stroke travels.** A quarter-outline `accent` dash on a 900 ms lap, frames asked for only while one travels, inside §10.2's budget. **Before it:** **`E3-T14` step A — the canvas shows a slow run reaching its nodes.** `CanvasNodeState.Evaluating`, marked after a 150 ms delay, cleared by each node's report and by the applied result; drawn as §7.4's static ring and `…` glyph. **Before it:** **`E3-T21` — closed on the evidence.** The graph-construction API was already public, and the CLI, collapse-to-custom-node and the tests already use it. **Before it:** **`E5-T10` — generic methods closed over `object` where a canvas can use them.** Constrained and uninferable ones refused by name; named instantiations waiting for an author. **Before it:** **`E5-T9` — extension methods shown on the type they extend.** `OwnerName` keys an extension as `Receiver.Method`; the rest of the node is the static method it is. **Before it:** **`E2-T13` step C — a decision: `Curve2d` waits for its consumer.** Filed as `E2-T64`; `E2-T13` closed on the planar pipeline. **Before it:** **`E2-T13` step B — offset and simplify.** `Region.Offset` with round, mitred or square corners; `Region.Simplify`. **Before it:** **`E2-T13` step A — `Region` and the planar booleans.** Closed polylines in a plane to a region and back; Clipper2 from one internal file. **Before it:** **`E2-T21` — `PointCloud`, and `GeometryGroup` declined.** A kept box and k-d-tree queries over a list; JSON and the viewport. **Before it:** **`E2-T16` step B — `Point.PruneDuplicates`.** `Point3d.PruneDuplicates` in the kernel over the tree, and the node over that. **Before it:** **`E2-T16` step A — a k-d tree over points.** Implicit, median-split, three deterministic queries, each against a linear scan. **Before it:** **`E2-T11` step C — the nodes.** `Curve.IntersectWith` gives the points; `Curve.OverlapWith` the shared stretches as pieces of the first curve. **Before it:** **`E2-T11` step B2 — one public entry point.** `Curve.IntersectWith`, closed form first; the three result types public, their API entries taken from RS0016. **Before it:** **`E2-T11` step B1 — the general curve/curve intersector.** Sample, prune through the BVH, refine by Gauss-Newton; points, touches and overlaps; agrees with step A on every closed-form pair. **Before it:** **`E8-T86` — the newest run wins, not the last to finish.** An older result reaching the apply gate after a newer one has been applied is dropped. **Before it:** **`E2-T11` step A — curve/curve intersection in closed form.** Lines, circles and arcs in 3D, with overlaps and tangency; internal until step B. **Before it:** **`E8-T12` — Settings.** `SettingsViewModel` over the existing preferences, one writer each; `SettingsWindow` from *File > Settings...* and `Ctrl+,`. **Before it:** **`E9-T7` step B2 — the viewport draws a node's geometry as the node finishes.** `SparkSession` threads the report through; the view model's `GeometryStream` publishes from it and the window repaints on `GeometryStreamed`. **Before it:** **`E9-T7` step B1 — the engine reports each node as it finishes.** An `IProgress<NodeCompleted>?` overload of `GraphEvaluator.Evaluate`, reported from `Run.Complete` outside the run's lock. **Before it:** **`E9-T7` step A — tessellation in parallel inside `SceneBuilder`.** Surfaces, solids, curves and meshes are prepared in a `Parallel.For` pass at the top of `Build` and emitted serially in key order, so the packages are the one-thread build's. **Before it:** **`E9-T8` step B — a click on geometry selects the node that drew it.** `ViewportControl.PickAt` and `GeometryPicked`; `MainWindowViewModel.SlotDrawing` turns the key back into a slot; the window calls `Canvas.SelectOnly`. **Before it:** **`E9-T8` step A — the ray through a pixel, and what it hits.** `Camera.RayThrough` from the look-at basis; `ViewportPicker.Pick` over the kernel's BVH, exact for triangles and five pixels forgiving for lines and points. **Before it:** **`E3-T9` — the evaluation cache gains a managed-memory budget.** A third ceiling over a count-based estimate that errs high and never materialises a resident shape; the native budget it sits beside had landed on 2026-08-31 and the register had missed it. **Before it:** **`E3-T12` step B — the evaluator's token reaches a library node that asks for it.** No port for a `CancellationToken` parameter, `NodeInvoker.ForCancellableMethod`, `NodeDefinition.InvokeCancellable`, and `Surface.ToMesh` declaring one ([D24](PRD.md#13-decision-log)); the code-example generator's one-port-per-parameter assumption fixed ([N155](NOTES.md)). **Before it:** **`E3-T12` step A — the kernel's long loop takes a cancellation token.** `Tessellate` and `ToMesh` overloads, checked per refinement span and per grid row; RS0027 off beside RS0026, and sixteen test calls patched from xUnit1051's own list ([N154](NOTES.md)). **Before it:** **`E8-T13` — autosave and crash recovery.** `RecoveryStore` in `Spark.Host`, written from `RefreshHistory` and `MarkSaved`, offered back by a banner with **Restore** and **Discard**, never silently; off in the view model until the window turns it on ([N153](NOTES.md)). **Before it:** **`E3-T10` — closed by reconciliation.** The impure-node declaration is `[NodeSideEffect]`, read by the importer, mixed into `CacheKey`, and tested by `EvaluationTests.AnImpureNodeReRunsEveryTimeAndPoisonsEverythingDownstream`; the row's re-check had searched for `Impure`. **Before it:** **`E2-T63` — a point exactly on a fold is its own closest point.** The reseed stepped off the rim and the line search compared with the seed, so the one step that mattered was refused; `Descend` now compares with the current point and `Refine` keeps the best apart. A step stretch was tried and taken out (it made another point worse by eight orders). **Before it:** **`E2-T62` — a closest point no longer stops on a saddle.** A trace found Newton overshooting onto a revolution surface's fold, clamping, and converging on a saddle; `Surface.EscapeSaddle` checks a converged point's Hessian and moves off along its negative eigenvector. Two earlier hypotheses were measured, rejected and not shipped. **Before it:** **`E12-T20` — a tessellation is made at the tolerance asked for.** The shim's *copy* of a shape was a handle assignment that shared every face, and meshing writes onto the face ([N151](NOTES.md)); `OcctBrepKernel.Place` meshes a held shape in place once and every later tolerance a fresh import, with a per-shape, per-tolerance cache for repeats. **Before it:** **A register reconciliation.** Closed with evidence: `E8-T16`, `E8-T17`, `E12-T10`, `E11-T9`, `E11-T12`, `E11-T13`, `E3-T22`, `E10-T9`, `E10-T10`, `E11-T18`, `E1-T26`. Re-described: `E8-T14` (In progress: the generated-reference drill-down waits on the post-1.0 Help pass), `E11-T15` (N90's headless hang), `E1-T19` (Blocked with the CI rows). **Before it:** **`E12-T24` — `spark export` takes graphs with code blocks.** A factory when the document has code blocks, `AdmitPackages` before `Restore`, `--no-script` and `--trust-packages` with `run`'s meanings, and no file written on a refusal; `Export` takes its two streams and is tested for the first time. **Before it:** **`E3-T24` — a `.spark` file's text is written as typed.** `SparkFile` writes with `JavaScriptEncoder.UnsafeRelaxedJsonEscaping`; the one-time diff on an older file's first save is accepted rather than engineered away (D23, ADR-0026); an emoji is still an escaped surrogate pair because no encoder setting allows one ([N150](NOTES.md)). **Before it:** **`E7-T25` — the command line reads a graph's package folder.** `run` and `check` refuse an assembly nobody agreed to, naming it and its full hash; `--trust-packages` references for one run through `GraphPackageGate.AgreeOnce` and writes nothing; only a graph with code blocks consults the folder ([N149](NOTES.md) is why a DLL is held to a stricter rule than a code block). **Before it:** **`E6-T40` — `--open` asks before it runs, and so does a screenshot.** One adoption with `evaluate: run`; the capture skips its own run while trust is pending ([N148](NOTES.md)). **Before it:** **`E8-T85` — a warning carries a warning triangle**, the client's interruption, and the wrong *not evaluated* reason found in the same screenshot. **Before it:** **`E7-T19` — Save As carries the package folder.** `GraphPackages.CopyFolder`, merged without overwriting; the gate follows the file (`NoteSavedTo`). **Before it:** **`E7-T17` step B — the list is written, and checked before anything is built.** `GraphPackages.Entries` / `Absent` / `ToRecord`; the list recorded on both doors; the banner names absent packages; Save asks where before it serialises ([N147](NOTES.md)). **Before it:** **`E7-T17` step A — the format.** `GraphDocumentPackage`, the `packages` section first in the file, `PackagesFormatVersion = 5`, and `E7-T7` re-proved at the document level. **Before it:** **`E7-T24` — a library that arrives after the block that needs it.** `EvaluateAsync` compares the catalogue's version with the one the blocks were built against, and a move forces the rebuild and a fresh cache epoch; `SparkSession.ReferencesVersion()` reads it without loading Roslyn. **Before it:** **`E7-T16` — the graph-local package folder, consumed behind its trust gate.** `GraphPackageGate` in `Spark.Host`; `ReferenceCatalog.RemoveUnder` and a `Remove` that takes its namespaces with it; a forced rebuild and a fresh cache epoch on agreeing, because a block's key is written into the file and cannot see the catalogue. **Before it:** `E2-T40` step B — the least-squares plane fit, and the row closes. An internal `LeastSquares` helper rather than a method on `Plane`, because `Circle` and `Line` want the same arithmetic; closed form, so no eigensolver and no convergence tolerance; a **relative** degeneracy threshold ([N143](NOTES.md)); and a normal whose sign follows the winding of the points. **Before it:** step A, polar construction ([N142](NOTES.md) is the rule the two halves put side by side). **Before them:** the constructor flake ([N141](NOTES.md)), `E8-T40` ([N140](NOTES.md)), `E2-T33` / `E11-T10` ([N139](NOTES.md)). |
| **Working tree** | Clean. Build clean with zero warnings, format clean, **3394** tests over **ten** executables with zero failures and zero skips, docs harness green — all fourteen checks, with the residue budget exact at 300 — and the help-sample compiler green. No stashes. |
| **Next action** | **`E2-T72`'s first idea — there is no curve fitting of any kind.** `Line.ByBestFitThroughPoints`, `Arc.ByBestFitThroughPoints` and `Circle.ByBestFitThroughPoints` are all absent, and it is one idea rather than three items. **Taken ahead of `E2-T71`'s remaining families because the machinery is already built and already tested**: `LeastSquares` is internal, `Plane.FromBestFit` has used it since `E2-T40` step B, and `Curve.PlaneOf` started using it today — so this is three closed-form fits over a helper with a settled degeneracy rule, not a new numerical method. **The line fit is the covariance matrix's *largest* eigenvector where the plane fit takes its smallest**, which is the same adjugate arithmetic read the other way round, so `LeastSquares` gains a `TryFitLine` beside `TryFitPlane` rather than a second file. **The circle fit is the one with a decision in it**: a circle through scattered 3D points is two fits, not one — the plane first, then the circle within it — and **the algebraic fit (Kåsa) and the geometric fit (Gauss–Newton on the radial residual) give different answers on a short arc**, where the algebraic one is known to under-estimate the radius. Decide which, say why in the row, and **test on a short arc rather than a full circle**, because that is the only case that can tell them apart. **Then**: `E2-T71`'s remaining families — `PolyCurve`'s exact NURBS conversion, chord stepping, extension; projection and pull wait on `E2-T15`'s ray caster — then `E2-T72`'s other two ideas, the general two-curve fillet and periodic NURBS, then `E2-T66`…`E2-T69`. **`Q12`'s T-Splines decision needs the client, not a step.** |
| **Verify with** | **A named test that goes red when the branch is removed, proved by removing it** (AGENTS.md step 7) — and for a fit the branch that matters is the **degeneracy refusal**, because a least-squares fit *always* returns something. `Plane.FromBestFit` already refuses near-collinear input with a **relative** threshold ([N143](NOTES.md)), and the three curve fits need the same discipline: collinear points have no circle, coincident points have no line, and returning a plausible one is worse than refusing. **Pin the fits against an exact answer, not against themselves**: points sampled from a known circle must recover that circle's centre and radius to near machine precision, which is the assertion that separates a correct fit from one that merely converges. **Then perturb them** — the same points with noise — and assert the residual falls rather than asserting a number nobody can predict. **And the short arc is the case the row turns on**: a fit that is right on a full circle and wrong on a 20° arc is the documented failure of the algebraic method, so the test that decides the implementation has to be the short one. The three gates, and the residue budget **exact** and moved with the reason written in the exclusions history. |
