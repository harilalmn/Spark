...
| **Working on** | **`E7-T16` - the graph-local package folder, loaded only with consent.** [ADR-0024](adr/0024-graph-local-package-folder.md) has the design and the client settled the four questions. `<name>.packages` beside `<name>.spark`, one folder per NuGet package and loose `.dll` files for anything hand-added. **The trust gate is in this commit and not a follow-up**: a folder of DLLs beside a downloaded graph is remote code execution, loading is not passive - module initialisers and static constructors run, and the node importer reflects over types - and `E6-T16` already refuses to auto-run the *readable* version of the same hazard. |
...
| **Next action** | **Finish `E7-T16`.** `GraphPackages` finds `<name>.packages`, enumerates assemblies from both shapes - a NuGet extraction with `lib/<tfm>/`, preferring the framework match, and a bare `.dll` at the top - and hands them to the reference catalogue and the load context only once consent is recorded **per content hash**. `PackageStore` already takes its root as a constructor argument, so the store half is `new PackageStore(folder)` rather than a new type. **Then** the search half of `E7-T20`, which is what the client actually sees: the Packages window returns nothing for `nice3point` because the query is hard-coded to `tags:spark`. |
| **Verify with** | The three gates, plus tests: a folder beside a saved graph is found and its assemblies enumerated in both layouts; a graph with no folder is not an error; **nothing loads before consent**, which is the assertion the whole row exists for; consent is remembered per hash and asked again when the bytes change; and a code block can use a type from a hand-dropped DLL. |
