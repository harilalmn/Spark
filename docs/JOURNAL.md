...
**Last updated:** 2026-09-11 (`E2-T63` closed: a point exactly on a fold is its own closest point)
**Protocol version:** 2
...
| **Next action** | **Close `E3-T10` by reconciliation — it is already delivered under another name.** The row asks for an `[Impure]` attribute on `Spark.Api`, the importer reading it, and a test that a downstream key changes when an impure node is upstream. All three exist as **`[NodeSideEffect]`**: declared at `src/Spark.Api/NodeAttributes.cs:347`, read by `NodeImporter.cs:585` from the member or its declaring type into `NodeDefinition.IsSideEffect`, mixed into the key at `CacheKey.cs:86`, and tested by `EvaluationTests.AnImpureNodeReRunsEveryTimeAndPoisonsEverythingDownstream`, with the clock and console nodes declaring it (`DateAndTimeNodeTests`, `ConsoleFromCodeTests`). The row's re-check grepped for `Impure` and found nothing. **Read the test before closing on it**, name it on the row, and then take **`E8-T13`** (aggressive autosave and crash recovery). |
| **Verify with** | For `E3-T10`: the test named on the row is read and asserts a downstream key changing, not merely a flag being set; the summary re-derived by script; no code moves, so the suite stands at **3,118**. |
