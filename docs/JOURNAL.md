...
**Last updated:** 2026-09-13 (`E2-T45`: §3.6 mesh, in progress)
**Protocol version:** 2
...
| **Next action** | **Assess §3.6’s 65 mesh members against the assembly, member by member, and rewrite the section with a measured *Reachable* column** — the shape `E2-T42`, `E2-T43` and `E2-T44` established. **The row already names the finding to confirm or refute**: eight repair and remeshing members — `Repair`, `MakeWatertight`, `CloseCracks`, `Remesh`, `Reduce`, `Smooth`, `Explode`, `MakeHollow` — *appear nowhere in FR-51, E2-T20 or E2-T27*. **Check that against the assembly and the documents first**, because `E2-T42`’s row made exactly that kind of claim about curvature and was wrong. **And carry [N159](NOTES.md) into the assessment**: `IndexGroup` is Dynamo’s mesh FACE RECORD, so §3.6 is full of indices — a mesh index stays managed and never crosses the seam, which is why it is safe, and that difference should be said on the rows rather than assumed. Spark has `Mesh`, `MeshBuilder`, `MeshTopology`, `MeshFace`, `Mesh.Area`, `Mesh.Volume()` and `Mesh.Welded(tolerance)`; `IBrepKernel.Tessellate` makes one from a `Brep`. |
| **Verify with** | For `E2-T45`: the four parity checks green with the residue budget **exact**; §3.6’s per-type counts still agreeing with the manifest and its *Reachable* column measured from the manifest rather than estimated; the row’s tally in TASKS with the eight repair members’ claim either confirmed or corrected **in writing**; `docs/progress.html` regenerated by `scripts/build-progress.py`; and the gates — build clean, **3319** tests over ten executables, format clean. |
