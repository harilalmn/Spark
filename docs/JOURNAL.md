...
| **Next action** | **`E2-T43` — Dynamo capability parity: solids**, now that the register can name what the kernel delivers. 55 members over 5 types, §3.4, all `Unassessed`. **Expect most of it to be reachable**, and expect the reachable half to name `Spark.Api.IBrepKernel` rather than `Spark.Geometry`: `Union`, `Difference`, `Intersection`, `Extrude`, `Revolve`, `Loft`, `Sweep`, `Patch`, `Fillet`, `Chamfer`, `Shell`, `Split`, `Trim`, `Offset`, `Thicken`, `Draft`, `Sew` and `Heal` are all on that interface, which is the list §3.4 should be read against. **Two decisions are already made and must be stated on the rows rather than re-argued**: Spark has **no `Solid` type** and will not — one `Brep` with `IsSolid` as a query, because Dynamo's split is by closure and so tolerance-dependent (§3.4) — and the four primitives are `BrepPrimitives` factories rather than types, with their 14 parameter-recovery properties refused in §5. **Watch for the `IsSolid` shell-level limit** `E13-T18` and `PolySurface.ExtractSolids()` both hit: `Brep.IsSolid` asks about the whole model, so a `Brep` of two shells cannot say which is closed. |
| **Verify with** | For `E2-T43`: the four parity checks green with the residue budget exact and every `Done` row naming a member some delivering assembly declares; §3.4's per-type counts still agreeing and its *Reachable* column measured as §3.3's and §3.5's now are; the row's tally in TASKS; and the gates, suite at **3309**. |
