...
**Last updated:** 2026-09-11 (`E2-T13` step B: offset and simplify)
**Protocol version:** 2
...
| **Working on** | **`E2-T13` step C — `Curve2d`, scoped in writing first.** Steps A and B are committed: `Region`, its booleans, offset and simplify. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
...
| **Next action** | **`E2-T13` step C — scope `Curve2d` in writing before building it.** The row says `Curve2d` is *required for BRep trim curves in UV space*, and `BrepTrim`'s own remarks say a trim carries no pcurve because `Curve2d` does not exist. **Find first**: which kernel code would consume a pcurve today - trimmed-face tessellation is behind the OCCT seam (ADR-0021), so the managed kernel may have no consumer at all - and which 2D curve kinds a trim needs (lines and NURBS in UV cover what OCCT hands back). If nothing in the managed kernel would read a pcurve yet, record that the type waits for its consumer - the rule `PreferenceFile` states about building for one caller - move `Curve2d` to its own row beside the BRep work that needs it, and close `E2-T13` on the pipeline it delivered. Otherwise build the smallest `Curve2d` that consumer needs. |
| **Verify with** | For `E2-T13` step C: the decision written in the row and in EPICS; if built, each member watched red; the gates, and the suite total rises from **3247**. |
