...
| **Next action** | **Assess, do not implement.** 33 rows over `Vertex`, `Edge`, `Face`, `Loop`, `CoEdge` and `Topology`; 6 are `Done` and 27 `Unassessed`. Scouted already: `Topology.Faces` / `Edges` / `Vertices` are `Brep.Faces()` / `Edges()` / `Vertices()`; `Edge.CurveGeometry` is `BrepEdgeView.Curve`; `Edge.StartVertex` / `EndVertex` are `BrepEdge.Start` / `End`; `Edge.AdjacentFaces` is `BrepEdgeView.AdjacentFaces()`; `Loop.IsExternal` is `BrepLoopView.Kind`; `Loop.CoEdges` is `BrepLoopView.Trim` over `TrimCount`; `CoEdge.Reversed` is `BrepTrim.IsReversed`; `Vertex.PointGeometry` is `Brep.VertexPoint`. **The gaps are the deliverable of this step**, and they are all one shape — the model stores every relationship one way, so `Vertex.AdjacentEdges` / `AdjacentFaces`, `Edge.CoEdges`, `Loop.Face`, `CoEdge.Loop` / `Next` / `Previous` / `Partner` / `StartVertex` / `EndVertex` have no member to name. `CoEdge.ParameterCurve` is the pcurve `E2-T64` carries. **`Face.SurfaceGeometry()` is the trimmed face**, which is exactly the difference [N156](NOTES.md) found under `PolySurface.Surfaces()`, and must not be marked `Done` on the untrimmed `BrepFaceView.Surface`. File the navigation gap as one row and lower the residue budget if any Spark member becomes named. |
| **Verify with** | For `E2-T44` step A: the four parity checks green — in particular every new `Done` row naming a member `Spark.Geometry.dll` really declares (the rename-catcher) and the residue budget exact; §3.5's per-type counts unchanged; the row's new tally stated in TASKS; and the gates with the suite at **3284**. |
