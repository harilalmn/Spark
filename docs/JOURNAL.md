...
| **Working on** | **Nothing — between steps.** `E2-T43` closed: §3.4 assessed, 24 of 55, from *0 reachable* — and most of the movement is `E11-T30` rather than new code. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T45` — Dynamo capability parity: mesh.** §3.6, 65 members over `Mesh` (55) and `IndexGroup` (10), all `Unassessed`. **The row already names the finding to confirm or refute**: eight repair and remeshing members — `Repair`, `MakeWatertight`, `CloseCracks`, `Remesh`, `Reduce`, `Smooth`, `Explode`, `MakeHollow` — *appear nowhere in FR-51, E2-T20 or E2-T27*, and mesh repair and decimation are their own literature. **Check that claim against the assembly first**, because `E2-T42`'s row made exactly that kind of claim about curvature and was wrong. Spark has `Mesh`, `MeshBuilder`, `MeshTopology` (naked edges, closure), `MeshFace`, `Mesh.Area`, `Mesh.Volume()` and `Mesh.Welded(tolerance)`; `IBrepKernel.Tessellate` makes one from a `Brep`. **`IndexGroup`'s 10 are Dynamo's mesh face records** and §3.6 already maps them to face records on `Mesh`, so expect most of those ten to be shape difference rather than gap. **Two fabrication members (`GenerateSupport`, `MakeHollow`) are a product direction, not a geometry gap** — say so on the rows rather than leaving them to look like omissions. |
| **Verify with** | For `E2-T45`: the four parity checks green with the residue budget exact; §3.6's per-type counts still agreeing and its *Reachable* column measured; the row's tally in TASKS with the eight repair members' claim either confirmed or corrected in writing; and the gates, suite at **3309**. |
