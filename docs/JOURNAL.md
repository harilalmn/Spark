...
**Left for `E2-T71`.** `Spark.Geometry.Occt.ModelWriter` has no word for a helix, so it falls to the
`default` branch and exports one by interpolating 64 samples with `Approximated` set. That is honest
and it is also the sharpest case yet for `Curve.ToNurbsCurve`, because a helix has an **exact**
rational form — a circle's knot structure with the control points lifted linearly along the axis.
Recorded rather than fixed here: it is `E2-T71`'s row, not this one's.
