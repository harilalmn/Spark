...
**Last updated:** 2026-09-13 (`E2-T45`: §3.6's 65 mesh rows assessed, 41 reachable)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps.** The last step assessed the mesh section and found the register wrong twice more, both in the *safe* direction — a whole assembly the rename-catcher was not reading, and an exclusion whose true premise reached a false conclusion ([N160](NOTES.md)). **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T46` — Dynamo capability parity: infrastructure**, the fifth and easiest subsystem assessment: **89 members over 8 types, of which 56 are already `Not planned` in the manifest and 33 are `Unassessed`.** §3.8. The 33 are `Geometry`'s own members, and the row already says where they split: **the transformation family (12) is `Transform` applied through `TransformedBy`/`Of*`** — `E2-T45` set that precedent on real rows a day earlier, so follow it rather than re-deciding it; **the measurement and intersection family (10)**, five of which are §6.1's exact-boolean count and the rest `BoundingBox`, `ClosestPointTo`, `DistanceTo`, `Explode` and `OrientedBoundingBox`; and **`ToJson`/`FromJson` are `GeometryJson.Serialize`/`Deserialize`**, which `E2-T45` proved by un-excusing the type. **Read the assembly, never the document** — four assessments in four days have each found the register claiming *absent* about something that shipped weeks ago ([N160](NOTES.md)), and §3.8 has had no pass at all. Expect the residue to move in both directions again, and write down which. |
| **Verify with** | For `E2-T46`: the four parity checks green with the residue budget **exact**; §3.8's per-type counts still agreeing with the manifest and a *Reachable* column measured from it rather than estimated; §5's refusal count re-derived if any of the 56 changes; `docs/progress.html` regenerated by `scripts/build-progress.py`; and the gates — build clean with zero warnings, **3319** tests over ten executables with zero failures and zero skips, format clean. |
