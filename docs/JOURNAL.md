...
**Last updated:** 2026-09-11 (`E9-T7` step B1: the engine reports each node as it finishes)
**Protocol version:** 2
...
| **Working on** | **`E9-T7` step B2 — the viewport draws a node's geometry as the node finishes.** B1 is committed: `GraphEvaluator.Evaluate` reports `NodeCompleted` from `Run.Complete`. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*.** |
