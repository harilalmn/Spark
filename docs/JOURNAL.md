...
| **Working on** | **Nothing — between steps.** `E11-T23` closed on 2026-09-12 with step B, and the Dynamo parity register now has a guard in both directions. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T44` — Dynamo capability parity: topology.** TODO item 10 is now the top of the list, and `E2-T44` is the tractable end of it: 33 members over 6 types, almost all of it navigation, and the row itself says full parity is achievable and should be. **The manifest is the worklist now** — filter `tests/corpus/dynamo-parity.tsv` to the six §3.5 types (`Vertex`, `Edge`, `Face`, `Loop`, `CoEdge`, `Topology`) and take each `Unassessed` row to a real status: `Done` with the Spark member and what was checked, or `Not planned` with a reason. The two structural differences are already decided and must be stated on the rows they touch, not re-argued: index-based rather than object-graph (`E2-T22`), and `CoEdge` → `BrepTrim`. `Face.SurfaceGeometry()` and `Edge.CurveGeometry` are the load-bearing members. **Expect the residue budget to fall**, and lower it in the exclusions file as part of the same step. |
| **Verify with** | For `E2-T44`: the four parity checks green with the budget lowered to its new exact value; the row's count of what is `Done`, `Not planned` and still `Unassessed` stated in TASKS; DYNAMO-COVERAGE §3.5's per-type counts still agreeing with the manifest; and the gates, with the suite total at **3284** or above. |
