...
**Last updated:** 2026-09-13 (`E2-T73` closes: `Helix`, the eighth curve type)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps.** Eight steps landed today, and the eighth is the first that **builds** what the register found rather than measuring it. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T71` and `E2-T72` — the curve gaps §3.2 found, and they are ideas rather than lists.** Four of them. **(1) No curve-to-NURBS conversion anywhere** (`Curve.ToNurbsCurve`), while `SurfaceConversion` has converted five of eight analytic surfaces exactly since M5 — and **`Helix` is what decides the *shape* of it**, because **a helix is provably not exactly representable as a NURBS curve** while every other curve Spark has is ([N165](NOTES.md)) — so `ToNurbsCurve` cannot return a bare `NurbsCurve` and stay honest. `Spark.Geometry.Occt.ModelWriter` already exports a helix through its `default` branch, interpolating 64 samples and setting `Approximated`; that is a limit to label, not a gap to close. **(2) No curve fitting of any kind** — `Line`, `Arc` and `Circle` each want a best-fit constructor and none exists, though `Plane.FromBestFit` and `NurbsCurve.FitPoints` do; `LeastSquares` is already internal and is where the arithmetic belongs. **(3) One fillet, and it takes two lines** — `CurveOffset.FilletLines` and nothing else, where four rows want the general two-curve fillet. **(4) No periodic NURBS curve**, because `KnotVector` is clamped and has only `CreateClamped` — [N156](NOTES.md)'s trap a third time, since `IsClosed` is not `IsPeriodic`. Take them in that order: (1) is the one another type already depends on. **Then**: `E2-T66`…`E2-T69` — no sphere and no cone in `BrepPrimitives`, no `Solid.Centroid`, the six missing mesh-repair members. **`Q12`'s T-Splines decision needs the client, not a step**, and `Q16`/`Q17` likewise. |
| **Verify with** | **A named test that goes red when the branch is removed, proved by removing it** (AGENTS.md step 7) — and for a conversion, the test that matters is that the converted curve **agrees with the original it was converted from**, sampled densely, rather than that it has the right number of control points. An exact conversion has to be asserted *as exact*: [N161](NOTES.md)'s lesson in the other direction, where a tolerance-sized agreement would hide a conversion that is merely a good approximation. `Helix` and `Circle` are the two anchors, because both have closed forms this repository does not otherwise contain. The three gates, the residue budget **exact** and moved with the reason written in the exclusions history — [N163](NOTES.md) says a rise is now the only thing that can mean anything, and [N164](NOTES.md) says the thing that catches a stale hand-written list is never where you would look for it. |
