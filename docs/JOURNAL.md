...
**Last updated:** 2026-09-13 (`E2-T41` step A in progress: §3.2's 82 `Curve` rows)
**Protocol version:** 2
...
| **Working on** | **`E2-T41` step A — §3.2's `Curve` base type, all 82 rows.** The largest single type in the inventory, assessed member by member against the **assemblies** ([N160](NOTES.md)) rather than against this document. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
...
| **Next action** | **Assess all 82 `Curve` rows in `tests/corpus/dynamo-parity.tsv`**, each with a status and a written reason, then rewrite §3.2's table with a **measured** *Reachable* column for `Curve` and update §2's tally and the residue budget. **Three calls this step has to make and must not dodge.** (1) §5 [i]'s duplicated trim family is *uncounted* because nobody chose the survivor — choose it, or say why it still cannot be chosen. (2) §6.3 names five `Curve` members whose behaviour a signature does not determine; they are `Needs a decision`, never a guess ([N156](NOTES.md)). (3) Where a capability is composed rather than named — `Trimmed` twice is a split — say so in the reason, because §2's own rule counts composition and a silent claim is indistinguishable from drift. File whatever is genuinely missing as a new task and leave `E2-T41` `In progress` for step B's ten concrete curve types. |
