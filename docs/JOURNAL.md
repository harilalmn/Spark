...
| **Next action** | **Assess `Surface`'s 46 rows; `NurbsSurface` and `PolySurface` are step B.** Scouted already. **The evaluation family is there and is most of what is reachable**: `PointAt`, `NormalAt`, `DerivativeAt` (which answers `TangentAtUParameter` and `TangentAtVParameter` too), `FrameAt` for `CoordinateSystemAtParameter`, `ClosestPoint` with its `out u, out v` for `UVParameterAtPoint`, `IsoCurveU` / `IsoCurveV` for `GetIsoline`, `IsClosedU` / `IsClosedV`, `Area`, `GaussianCurvature`, `MeanCurvature` and **`PrincipalCurvatures`, which the row's own text said was missing and is not** — check that claim and correct the row. `PrincipalDirectionsAtParameter` really is absent. `RevolutionSurface` and `RuledSurface` answer `ByRevolve` and the two-curve `ByRuledLoft`. **The finding to write up carefully**: `Spark.Geometry` declares no loft, sweep, thicken, surface boolean, fillet or chamfer, because `IBrepKernel` is in **`Spark.Api`** — so a capability Spark really delivers cannot be marked `Done`, since the rename-catcher only reads `Spark.Geometry.dll`. **Do not fudge it and do not widen the check in this step**: assess those rows honestly against the rule as it stands, and file the register's one-assembly limit as its own row. Use [DYNAMO-COVERAGE](DYNAMO-COVERAGE.md)'s own rule that composition counts — *`Vector.Transform(cs)` is counted because `CoordinateSystem.ToWorld` and `Transform.OfVector` between them do the job* — and say in each reason which members compose. |
| **Verify with** | For `E2-T42` step A: the four parity checks green with the residue budget exact; every new `Done` row naming a member `Spark.Geometry.dll` declares; §3.3's per-type counts unchanged; the row's tally stated in TASKS and the one-assembly limit filed; and the gates with the suite at **3309**. |
