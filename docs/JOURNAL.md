...
| **Next action** | **`E11-T30` — make the parity register read the assemblies that deliver the geometry**, rather than `E2-T43` next in the subsystem order. **Twenty rows across §3.3 now say `Planned` about capabilities that already work** — loft, sweep, thicken, the booleans, fillet, chamfer — and `E2-T43` solids will add more of the same, because solids are where the kernel does most of its work. Assessing §3.4 before fixing the scope means writing that lie a third time. **Decide the rule first**: §1 and FR-81 both say the register's subject is *capability*, so the rename-catcher should read `Spark.Geometry`, `Spark.Api` and `Spark.Nodes.Core` and a row should name whichever holds the member. **Keep the reverse direction on `Spark.Geometry` alone** — its residue budget is a measure of *that* assembly's surface against the plan, and `Spark.Nodes.Core`'s node families would swamp it. `Spark.Docs.Verify` must still reference no Spark project and read every assembly from disk as metadata. **Test first**: a Done row naming a `Spark.Api` member is accepted, one naming a member no assembly declares is still refused by name. Then re-assess the twenty rows. |
| **Verify with** | For `E11-T30`: the rename-catcher watched red on a `Spark.Api` member that does not exist and green on one that does; the reverse direction's residue budget unchanged, which is the proof it stayed scoped to `Spark.Geometry`; §3.3's *Reachable* column rising by the twenty as they are re-assessed; and the gates, suite at **3309** or above. |
