...
| **Working on** | **Nothing — between steps.** The step set out to make a demo visible and found a kernel defect instead: **a face or edge index does not survive the crossing into OCCT** (`E13-T22`). **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T45` — Dynamo capability parity: mesh**, as before. §3.6, 65 members over `Mesh` (55) and `IndexGroup` (10), all `Unassessed`. **The row already names the finding to confirm or refute**: eight repair and remeshing members — `Repair`, `MakeWatertight`, `CloseCracks`, `Remesh`, `Reduce`, `Smooth`, `Explode`, `MakeHollow` — *appear nowhere in FR-51, E2-T20 or E2-T27*. **Check that against the assembly first**, because `E2-T42`'s row made exactly that kind of claim about curvature and was wrong. **And carry [N159](NOTES.md) into the assessment**: `IndexGroup` is Dynamo's mesh FACE RECORD, so §3.6 is full of indices — a mesh index stays managed and never crosses the seam, which is why it is safe, and that difference should be said on the rows rather than assumed. Spark has `Mesh`, `MeshBuilder`, `MeshTopology`, `MeshFace`, `Mesh.Area`, `Mesh.Volume()` and `Mesh.Welded(tolerance)`; `IBrepKernel.Tessellate` makes one from a `Brep`. |
| **Verify with** | For `E2-T45`: the four parity checks green with the residue budget exact; §3.6's per-type counts still agreeing and its *Reachable* column measured; the row's tally in TASKS with the eight repair members' claim either confirmed or corrected in writing; and the gates, suite at **3314**. |
