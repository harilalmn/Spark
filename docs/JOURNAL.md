...
| **Working on** | **Nothing — between steps.** Seven steps landed today: four register assessments that finished it, and three pieces of the geometry they found missing. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T73` — `Helix`, the eighth curve type**, decided in `D27` on 2026-09-13 and the cheapest type this hierarchy can gain. **A helix travels at a constant speed**, so `LengthAt`, `ParameterAtLength`, `PointAtLength`, `DivideEqually` and `DivideByLength` are all closed form and exactly right — which is the whole argument, and it is also the trap: **every curve except the ellipse travels at a constant speed, so a division test cannot tell a right answer from a plausible one on any of them** (`E2-T7` learnt that once already). Pin the arc length against the analytic `sqrt(r^2 + (pitch/2π)^2) · Δangle` rather than against the curve's own `DivideEqually`. **Seven Dynamo members**: `ByAxis(origin, direction, startPoint, pitch, angle)` and the parameter recovery over it — `AxisPoint`, `AxisDirection`, `Pitch`, `Radius`, `Angle` — plus `ToString`, and [N161](NOTES.md) says that last one is not optional: `Curve.ToString()`'s `Done` row rests on **all** of Spark's concrete curve types overriding it, and an eighth that does not would quietly falsify it. Needs `GeometryJson` serialisation (its sample test will demand one), a node family, a help topic with a worked example, an FR-48 entry, and the seven parity rows move from `Planned` to `Done`. **Then**: `E2-T71`/`E2-T72` — no curve fitting of any kind, one fillet that takes two lines, no periodic NURBS, no curve-to-NURBS conversion — then `E2-T66`…`E2-T69`. **`Q12`'s T-Splines decision needs the client, not a step.** |
| **Verify with** | **A named test that goes red when the branch is removed, proved by removing it** (AGENTS.md step 7) — the closed-form/general agreement is the one that matters. Plus: a line through a sphere giving two points at the right parameters; a line missing it giving none; a curve crossing a `NurbsSurface`; a crossing outside the surface's domain rejected; the points ordered along the curve. The three gates, the residue budget **exact** and raised by however many members this adds with the reason written in the exclusions history — [N162](NOTES.md) says a rise from new code is what the check is for, and [N163](NOTES.md) says a rise is now the only thing that can mean anything. |
