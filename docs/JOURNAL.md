...
**Last updated:** 2026-09-11 (`E6-T40` closed: `--open` and `--screenshot` ask before they run)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps, and `E6-T40` is closed.** Both doors apply `E6-T16`'s rule now — File ▸ Open and `--open` — and a screenshot no longer runs a graph nobody trusted. Today closed `E7-T16`, `E7-T24`, `E7-T17`, `E7-T19`, `E8-T85` and `E6-T40`; the client confirmed the `E7-T16` demo and the warning triangle. Open from today: `E7-T25` and `E3-T24`. |
| **Step status** | `CLEAN` |
| **Last completed step** | **`E6-T40` — `--open` asks before it runs, and so does a screenshot.** One adoption with `evaluate: run`; the capture skips its own run while trust is pending ([N148](NOTES.md)). **Before it:** **`E8-T85` — a warning carries a warning triangle**, the client's interruption, and the wrong *not evaluated* reason found in the same screenshot. **Before it:** **`E7-T19` — Save As carries the package folder.** `GraphPackages.CopyFolder`, merged without overwriting; the gate follows the file (`NoteSavedTo`). **Before it:** **`E7-T17` step B — the list is written, and checked before anything is built.** `GraphPackages.Entries` / `Absent` / `ToRecord`; the list recorded on both doors; the banner names absent packages; Save asks where before it serialises ([N147](NOTES.md)). **Before it:** **`E7-T17` step A — the format.** `GraphDocumentPackage`, the `packages` section first in the file, `PackagesFormatVersion = 5`, and `E7-T7` re-proved at the document level. **Before it:** **`E7-T24` — a library that arrives after the block that needs it.** `EvaluateAsync` compares the catalogue's version with the one the blocks were built against, and a move forces the rebuild and a fresh cache epoch; `SparkSession.ReferencesVersion()` reads it without loading Roslyn. **Before it:** **`E7-T16` — the graph-local package folder, consumed behind its trust gate.** `GraphPackageGate` in `Spark.Host`; `ReferenceCatalog.RemoveUnder` and a `Remove` that takes its namespaces with it; a forced rebuild and a fresh cache epoch on agreeing, because a block's key is written into the file and cannot see the catalogue. **Before it:** `E2-T40` step B — the least-squares plane fit, and the row closes. An internal `LeastSquares` helper rather than a method on `Plane`, because `Circle` and `Line` want the same arithmetic; closed form, so no eigensolver and no convergence tolerance; a **relative** degeneracy threshold ([N143](NOTES.md)); and a normal whose sign follows the winding of the points. **Before it:** step A, polar construction ([N142](NOTES.md) is the rule the two halves put side by side). **Before them:** the constructor flake ([N141](NOTES.md)), `E8-T40` ([N140](NOTES.md)), `E2-T33` / `E11-T10` ([N139](NOTES.md)). |
| **Working tree** | Clean. Build clean with zero warnings, format clean, **3,092** tests over **ten** executables with zero failures and zero skips, docs harness green. No stashes. |
| **Next action** | **`E7-T25` — `spark run` and `spark check` read a graph's package folder**, the last of the package-folder group. **The work is the trust question, not the wiring**: the command line has no banner to ask through. The row's shape: reference only what `PackageTrustStore` already records, name the rest in the refusal, and let the command line's existing explicit trust instruction agree to them **for that run only**, recording nothing. **Find first** how `spark run` refuses an untrusted graph today (`src/Spark.Cli/Program.cs`, the `E6-T16` comments near lines 155 and 314) and reuse that switch rather than inventing a second one. `GraphPackageGate` in `Spark.Host` should serve as it stands. **Write the refusal test first**, in `Spark.Cli.Tests`, as `E7-T16` did. **Alternative**: `E3-T24`, script text written with `+` and quotation marks escaped — a format decision about a one-time diff. |
| **Verify with** | For `E7-T25`: `spark check` on a graph whose folder holds an agreed assembly compiles its code blocks against it; a folder holding an unagreed one is named in the refusal and nothing from it is referenced; the explicit trust instruction lets it run once and records nothing. The gates, and the suite total rises from **3,092**. |
