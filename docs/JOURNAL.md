...
**Last updated:** 2026-09-11 (`E12-T24` closed: `spark export` takes graphs with code blocks)
**Protocol version:** 2
...
| **Next action** | **A register reconciliation of the open rows the tree appears to have already delivered**, because TODO's *Now* list is exhausted and the open list carries rows that read as done: `E8-T16` (both banners exist - `E6-T16`'s script banner and `E7-T17`'s missing-package banner), `E8-T17` (group and note rendering, which `E8-T6` built), `E12-T10` (the portable zip, which every release since `v0.1.0` has shipped), `E11-T9` (the reflection-driven round-trip test, queue item 5 on 2026-08-29 as `E2-T31`), `E11-T12` (the lacing case table) and `E11-T13` (the node-to-member diff). **For each, find the code or test that delivers it and name it on the row**, or rewrite the row to say in one sentence what is missing - never close one on recollection. Then TODO's *Now* list is rewritten from the register, and the step after is **`E12-T20`**, the tessellation cache that ignores the tolerance it was asked for, which is a defect rather than a feature. |
| **Verify with** | For the reconciliation: every row closed names a file and a test that a reader can open; the summary line re-derived by script; the gates unchanged at **3,107**, since no code moves. |
