...
**Last updated:** 2026-09-13 (`E2-T41` step B in progress: §3.2's ten concrete curve types)
**Protocol version:** 2
...
| **Working on** | **`E2-T41` step B — §3.2's ten concrete curve types, 105 rows.** `Line` 6, `Arc` 14, `Circle` 8, `Ellipse` 8, `EllipseArc` 9, `Helix` 7, `NurbsCurve` 15, `PolyCurve` 21, `Polygon` 9, `Rectangle` 8 — the rest of §3.2, and the row closes with it. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
...
| **Next action** | **Assess all 105 rows of the ten concrete curve types**, each with a status and a written reason, then finish §3.2's *Reachable* column, update §2's tally and the residue budget, and close `E2-T41`. **These rows are mostly constructors and parameter recovery**, so [N161](NOTES.md) is the whole job — with ten types in one step, every `Done` row that holds for *some* types must enumerate the ones it does not hold for, in writing. **[N162](NOTES.md) says to expect the residue to RISE**, because these capabilities are constructions and a `Done` row naming a bare type un-excuses its whole surface; say the number in the budget entry either way. **Three decisions live in these rows and must not be dodged**: `Helix` as a type, absent from FR-48 and undecided since the document was written; whether `Rectangle` and `Polygon` stay `PolyLine` factories rather than types, which §3.2 argues and no row records; and `PolyCurve.Heal(trimLength)` with `CurveAtIndex(index, endOrStart)`, which §6.3 names as undeducible. |
