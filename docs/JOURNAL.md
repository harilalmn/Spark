...
**Last updated:** 2026-09-11 (`E5-T10` closed: generic methods)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps, and `E5-T10` is closed.** Next is `E3-T21`, the public graph-construction API. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
...
| **Next action** | **`E3-T21` — the public graph-construction API.** The row says it is *needed by the CLI, by tests, and by collapse-to-custom-node*. **Find first what already exists**, because tests already build graphs with `Graph.AddNode`, `Graph.SetLiteral` and `Graph.TryConnect`, and the register has been stale before: list `Graph`'s public mutating members, see what the CLI and the canvas's collapse-to-custom-node actually call, and read the row's note for what it asked beyond them (a fluent builder? construction from a `.spark` document? by node name rather than definition?). If the API a caller needs is already public and tested, close the row on the evidence with the gap, if any, named; otherwise add the smallest missing piece, test first. |
| **Verify with** | For `E3-T21`: the evidence or the new piece, watched red; the gates, and the suite total rises from **3254** if anything is added. |
