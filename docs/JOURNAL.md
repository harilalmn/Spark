...
| **Working on** | **`E2-T43` — Dynamo capability parity: solids.** §3.4's 55 rows over five types, assessed against the kernel interface the register can now name. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
