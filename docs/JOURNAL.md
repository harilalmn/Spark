...
**Last updated:** 2026-09-01 15:10 +0530
**Protocol version:** 2
...
| **Step status** | `CLEAN` |
| **Last completed step** | **`E6-T25` — a code block's output port carries the type its script returns.** Found by a user drawing a wire: a block returning a `Circle` displayed the circle in its own watch and could not be connected to anything that wanted a `Curve`, because every output port was `typeof(object)` and `object` into `Curve` is a narrowing that `TypeCompatibility` refuses on purpose. The type now comes off the semantic model of the compilation that is emitted anyway, and **the disk cache carries it**, or a port would be `Circle` in the session that compiled it and `object` in the one that reopened the file ([N99](NOTES.md)). **2,340 tests.** |
