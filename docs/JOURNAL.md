...
| **Working on** | **Nothing — between steps.** `E2-T65` and with it `E2-T44` closed: topology parity stands at **31 of 33**, and the two left are `E2-T64`'s pcurves. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T42` — Dynamo capability parity: surfaces**, taken the way `E2-T44` was: **assess first, then build what the assessment finds**, because topology's twelve gaps turned out to be one gap and the surface ones may cluster the same way. 106 members over 5 types, of which `PanelSurface`'s 25 are already refused (§5 [d]) — so about 81 rows, all `Unassessed` today. **Of `Surface`'s 46, twenty-two are behind the exact-boolean seam (`E2-T47`) and 24 are pure evaluation**, which is where the reachable ones will be. **Expect the curvature family** — Gaussian, principal values, principal directions — to be missing: `E2-T17`'s contract names ten members and not those, and a facade or structural graph reaches for them immediately. **Write the assessment into the manifest rows with what was checked, not a name match** ([N156](NOTES.md) is why), file what is missing as rows, and rewrite §3.3 with a measured *Reachable* column as §3.5 now has. The residue budget will move; put the reason in its `Reason` cell ([N157](NOTES.md)). |
| **Verify with** | For `E2-T42`: the four parity checks green with the residue budget exact and every new `Done` row naming a member `Spark.Geometry.dll` declares; §3.3's per-type counts still agreeing with the manifest; the row's tally of `Done`, `Not planned` and `Unassessed` stated in TASKS; and the gates, suite at **3309** or above. |
