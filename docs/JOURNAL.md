...
**Last updated:** 2026-09-11 (`E3-T20` step B: spark pack)
**Protocol version:** 2
...
| **Working on** | **Nothing — between steps.** `E3-T20` step B is committed: `spark pack`, and bundles accepted by `run`, `check` and `export`. Next is step C, the application. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
