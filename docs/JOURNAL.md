...
**Last updated:** 2026-09-13 (`E2-T71` step B in progress: the rational curves, and one copy of the quarter-circle)
**Protocol version:** 2
...
| **Working on** | **`E2-T71` step B — `Arc`, `Circle` and `EllipseCurve` convert exactly, and `SurfaceConversion`'s quarter-circle arithmetic stops being private to it.** **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
