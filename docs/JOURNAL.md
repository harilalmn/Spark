...
| **Working on** | **Nothing — between steps.** `E2-T44` step A landed: §3.5's 33 topology members assessed one by one, and the 12 that are not reachable turn out to be a single gap, filed as `E2-T65`. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
| **Step status** | `CLEAN` |
...
| **Next action** | **`E2-T65` — Brep topology navigation in the direction the model does not store**, which is `E2-T44` step B and closes that row. Twelve §3.5 members want it. **The design question is the whole of the task and it is already half-answered in the code**: `BrepEdgeView.AdjacentFaces`'s remarks argue that a back-pointer array on the records would be a second description of the same fact that has to be kept in step, and that a caller wanting the reverse repeatedly should build the index once and keep it. **So build that index as a public type** — a `BrepAdjacency` constructed from a `Brep` in one pass, holding vertex→edges, edge→trims and loop→face — rather than putting fields on `BrepEdge` and friends, which would turn the kernel into the object graph `E2-T22` decided against. `CoEdge.Next` / `Previous` / `StartVertex` / `EndVertex` need no index at all: trims are contiguous from `BrepLoop.FirstTrim`, so they belong on `BrepLoopView` as arithmetic that wraps at `TrimCount`. **Test first**, and against a `Brep` with an inner loop, because a face with a hole is where outer/inner and trim-direction mistakes show. Then re-point `BrepEdgeView.AdjacentFaces` and `FaceOf` at it, assess the twelve rows to `Done`, and lower the residue budget. |
| **Verify with** | For `E2-T65`: a named kernel test per new member, at least one over a face with an inner loop; `BrepEdgeView.AdjacentFaces` still answering exactly as before (its existing tests unchanged and green); the four parity checks with §3.5's *Reachable* at 31 of 33 and the residue budget lowered to its new exact value; and the gates, suite above **3284**. |
