...
| **Next action** | **`E2-T42` step B — `NurbsSurface`'s 17 rows and `PolySurface`'s 18.** The rest of §3.3, and the smaller half. **`NurbsSurface` should go almost all `Done`**: `DegreeU` / `DegreeV`, `IsRational`, `ControlPoints()`, `Weights()` and `ToString()` already are, and `NumControlPointsU` / `V` are `ControlPointCountU` / `V`, `UKnots` / `VKnots` are `KnotsU` / `KnotsV` on `KnotVector`, and `IsPeriodicInU` / `V` need checking against `IsClosedU` / `IsClosedV`, which are **not** the same question - closed is geometric, periodic is a property of the knot vector, and marking one as the other would be exactly [N156](NOTES.md)'s mistake. The `By*` constructors map to `NurbsSurface`'s own, so those rows name the **type** - which is now caught by the wholesale-excuse rule, so expect `NurbsSurface` to leave the excused list if it is on it. **`PolySurface` is a `Brep` with an open shell**: `EdgeCount`, `VertexCount` and `ToString` are `Done`; `SurfaceCount()` is `Brep.FaceCount`; `Surfaces()` is the one [N156](NOTES.md) already demoted, trimmed against untrimmed; `UnconnectedBoundaries()` is the naked edges, which `BrepAdjacency.TrimsOf` now answers; and the `By*`, `Fillet`, `Chamfer` and `ExtractSolids` rows are `E11-T30`'s seam again. Lower the residue budget and write the reason in its `Reason` cell. |
| **Verify with** | For `E2-T42` step B: the four parity checks green with the residue budget exact and every `Done` row naming something `Spark.Geometry.dll` declares; §3.3's per-type counts still agreeing and its *Reachable* column updated for both types; the row closing with its tally in TASKS; and the gates, suite at **3309**. |
