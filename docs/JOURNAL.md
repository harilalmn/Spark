...
| **Working on** | **`E2-T42` step B — `NurbsSurface`'s 17 rows and `PolySurface`'s 18**, which closes §3.3 and the row. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*: no tag or release until the register is worked out.** |
