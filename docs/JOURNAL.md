...
**Last updated:** 2026-09-13 (`E2-T40` in progress: §3.1's 78 unassessed value rows)
**Protocol version:** 2
...
| **Working on** | **`E2-T40` — §3.1, the value layer, and the last assessment in the register.** 78 `Unassessed` rows over `Point` 7, `Vector` 17, `Plane` 10, `CoordinateSystem` 28, `BoundingBox` 15 and `UV` 1. **Unlike the six sections before it, this one is checking a number rather than replacing one**: §3.1's *99 reachable* is the figure the whole document was built on and it was counted by hand, in prose, a year ago. **Run parameters from the client, 2026-09-11: *go non stop till all Epics are done*, then *finish everything - we release only after that*.** |
...
| **Next action** | **Assess the 78 rows**, each with a status and a written reason, then check §3.1's hand count against the manifest and rewrite the section with a measured *Reachable* column. **Expect OVER-claims here where every other section had under-claims**, because this is the one subsystem the original count was made of, and say plainly if the 99 does not survive. **`CoordinateSystem` is 28 rows and 12 of its 46 are already refused under §5 [h]** — check those refusals line up with the section's prose before counting anything. Two rules carry: read the **assembly** ([N160](NOTES.md)), and **enumerate what does not carry a member** ([N161](NOTES.md)). `E2-T40` is marked `Done` and was closed on three small omissions rather than on a member-by-member pass — **if the assessment contradicts that, correct the row rather than the evidence**. |
