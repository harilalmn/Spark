...
| **Working on** | **`E7-T21`, half landed.** `PrepareLibraryAsync` downloads a package and its dependencies with no manifest required, the refusal message names the way out, and an added library's namespaces are imported so no `using` is typed. **The button does not exist yet**, so the improved message currently points at an action the UI cannot take - which is worse than before in that one narrow way and is why this row is not Done. |
...
| **Next action** | **Add the *Add as a library* button**, putting the assemblies in the graph's own `<name>.packages` folder rather than the global reference store - the design the client settled in [ADR-0024](adr/0024-graph-local-package-folder.md), which also means the button is disabled until the file is saved, as `E7-T18` specifies. Building the global-store version first would be a path written to be removed. |
| **Verify with** | The three gates, plus: a package with no manifest is added as a library rather than refused; its dependencies come with it; a code block uses one of its types **with no `using` typed**; and the refusal message, when it still appears, says what can be done rather than explaining a convention the reader has no use for. **And the application**, with the client's own `nice3point` search. |
