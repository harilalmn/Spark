...
- **What is not.** No split, trim, thicken, draft or offset on the kernel contract. No mesh
  booleans. Trimmed faces come *back* from the provider but cannot be authored. **All seven `spark`
  verbs now exist**, the last four of them on 2026-09-15; the CLI dispatches `run`, `check`,
  `export`, `render`, `pkg`, `graph`, `docs`, `pack` and `--version`. **No surface or solid property tests**: CsCheck
...
> 3. ~~**`E3-T10`** — the impure-node declaration.~~ **Closed 2026-09-11 by reconciliation**: it
>    was delivered as `[NodeSideEffect]`, with the importer, the cache key and the downstream test
...
>    calls cannot be interrupted) — and ~~**`E3-T9`**'s byte budget~~ **done 2026-09-11**: the native half had been there since
...
> 7. ~~**`E9-T8`**~~ and ~~**`E9-T7`**~~ **done 2026-09-11**: a click on geometry selects its node,
...
>     cluster. ~~**`E2-T42`**~~ **done 2026-09-12**: all 81 non-`PanelSurface` rows of §3.3
>     assessed, **36 of 106 reachable**, and the evaluation family found **complete** rather than
>     missing its curvature half - only the principal *directions* are absent. **It also found the
>     register lying in the safe direction**: twenty rows say `Planned` for loft, sweep, thicken,
>     the booleans, fillet and chamfer, which Spark delivers behind `Spark.Api.IBrepKernel`,
...
>     the row's order. ~~**`Surface.ToNurbsSurface`**~~ **done the same day**: on the base, so a
>     `Surface`-typed value can be asked at all, **and the three types that had no conversion —
>     extrusion, revolution, ruled — now have one**, each exact when the curves it is built from
>     are. **The finding is that *exact* was two claims wearing one name**: a ruled surface joins
>     its rails at *equal parameters*, so two rails that are each the same curve under a different
>     parameterisation rule a **different sheet** — `NurbsConversion` and its new surface twin now
...
>     ~~**`ApproximateWithTolerance`**~~ **done the same day, with `NurbsSurface.ByPoints`
>     underneath it** — two rows and one algorithm, because an approximation *is* sample a grid
>     and interpolate it. The interpolation is the tensor-product form of the curve's, the same
>     banded solve applied along one direction and then the other. **The branch is where the
>     deviation is measured**: at the sample points it is zero by construction, so a fit that
...
>     `E2-T66`. ~~**`Surface.Join`**~~ **done 2026-09-14, and `E2-T66` is closed**: the row's own
>     blocker was that a surface has to become a face with a loop first, so the step was
>     **`Brep.FromSurface`** — a single face bounded by the surface's own four iso-curve
>     boundaries — and the join on top of it. **A degenerate boundary is dropped rather than
>     refused**, so a whole sphere's loop closes with three edges, which is this kernel's existing
...
>     **What is left of `E2-T68`**: `Repair`, `MakeWatertight`, `Remesh` and `Reduce`.
>     ~~**`Polygon.SelfIntersections`**~~ **done the same day**: a curve can now be asked about
>     itself, which nothing in Spark could do — `Curve.IntersectWith` takes *another* curve, and
>     a closed outline crossing itself is the commonest real case. **The branch is the closing
>     segment**: skip only the `i, i + 1` pairs and every rectangle reports a crossing at its own
>     first corner. ~~**And two `Arc` constructors**~~ **done the same day**: centre/start/end,
>     which is **over-determined** — the end point gives a direction and not a distance, and the
>     remarks say so — and start/end/tangent, which is well posed and passes through both points.
>     **What is left of `E2-T72`** is `PolyCurve` work: thickening, grouping, `BasePlane`, the
>     flagged offset, and the cyclic periodic interpolation. `Q12`'s T-Splines decision is 169 members
...
>     machinery.
> 11. ~~**`E2-T38`** — golden-file geometry tests with readable diff tables.~~ **Done
>     2026-09-15, and delivered by `E11-T11` the day before.** Closed by *corrupting a real
>     golden and reading the failure*, which is the only way to judge a diagnostic: the table
>     prints `field / golden / actual / delta`, marks the rows that moved, keeps the ones that
>     did not, and carries all four statistics the row named. **Doing it that way found the
>     defect that reading the source would not have** — the `.actual.tsv` the report names
>     survived the run that went green again, and being gitignored it then stood as evidence
...
> 16. **`E11-T32` extended so the blind spot is closed in code, not only written down.**
>     Ten boxes were ticked by hand on 2026-09-15 and were guarded by nothing;
>     `AcceptanceCriterionChecks` now ticks a multi-row box **iff every cited row is
>     `Done`**. Writing it forced a definition of *cites*, and the obvious one was wrong —
>     a criterion's prose names rows it does not depend on. **Measured over all 219: all 208
>     row-citing criteria cite inside a parenthesis and none only outside**, so reading
>     citations from the parenthesis loses nothing and recovers **18 single-row criteria**
>     that were covered by nothing at all. The two sets are now asserted to *exactly cover*
>     every row-citing criterion. Six disagreements on the first run: one real tick (the
>     OCCT licence obligations) and five legitimate exemptions with reasons. Proved in both
>     directions. Harness 67 → **68** checks.
> 17. **`E11-T34`** — the kernel's shape rules are guarded rather than grepped. **Done
>     2026-09-15.** Two `EPICS.md` boxes assert something about the *code* and were ticked
>     that morning on a search run once; both cite **no register row**, so no version of the
>     criterion check could reach them. Nine of the eleven zero-row criteria are E1
>     scaffolding facts with a build setting or a test already behind them — these two had
>     nothing. `KernelShapeTests`, six tests: sealing, the `private protected` closed
...
> 19. ~~**`E2-T71`** — the curve gaps `E2-T41` step A found.~~ **Done 2026-09-15**, and the
>     row was stale in **five** places: three of its own and **two the morning's sweep wrote
>     into it** ([N183](NOTES.md)). `Curve.ToNurbsCurve` has eight implementations, not none;
>     chord stepping is all present; extension is `Curve.Extended`; the curve-wide
>     `Curve.Normal` is a **deliberate refusal** documented in `PlaneOf`'s remarks; and the
>     two extrude overloads are `Done` by composition. **A sweep may only correct what it
>     verified** — a re-dated wrong clause is worse than an old one, because it reads as
>     freshly checked. **What was genuinely left was one member and it is built**:
>     `CurveOffset.OffsetMany`, which offsets each segment and returns the pieces rather than
>     fitting one connected curve through a gap where the offset does not exist. Two defects
...
- [ ] **`E7-T16` … `E7-T20` — the graph-local package folder.** Asked for by the client, who drew
      ...
      3. **`E7-T18` and `E7-T19` are both done** — refuse the Packages window on an unsaved
         graph and say why; copy the folder on Save As (2026-09-11: copied, never moved, and merged
         without overwriting what is already there). Both are the client's calls and both remove
      ...
      4. **`E7-T20` and `E7-T21` are both done.** Install from nuget.org was the Visual Studio half
         ...
         not do. **[`E7-T25`](TASKS.md#e7--packages-and-extensibility) closed the group on
         2026-09-11**: `spark run` and `spark check` read the folder behind the same content-hash
         consent, refuse an assembly nobody agreed to by name and full hash, and take
         `--trust-packages` for one run, recording nothing.
...
- [x] ~~**`E9-T7`** — parallel streamed tessellation~~ and ~~picking through the kernel's BVH
      ray caster (`E9-T8`)~~, both done 2026-09-11. M2-era viewport work rather than anything M5 owed.
...
- [x] Geometry serialization v1 and the reflection-driven round-trip test — `E2-T29`, `E2-T31`,
      ...
      saveable.
- [x] **The C2VGeometry test harvest, timeboxed to one week with a hard stop** — `E2-T32`,
      **closed 2026-09-15 inside the timebox, in two passes, and the first file paid for both.** `CircularArcs.Bounds` had no
      ...
      nothing. Every remaining file was assessed and none of it transfers; the reasons are on the
      `E2-T32` row so nobody re-derives them. **The finding that explains the small yield**:
      DoodleSharp's sweep bugs came from storing a sweep as a pair of absolute angles, and Spark
      stores start plus signed sweep, so the family is unrepresentable except where the wrapped
      arithmetic is genuinely needed — and **both places that do it had holes**, the second being
      `AnalyticCurveIntersection`'s two tolerance branches.
...
- [x] **Close the value-layer parity gaps** — `E2-T40`, **closed 2026-09-10**. Three small ones
      ...
      2026-09-10. **`Plane` is 16 of 16 — the first ProtoGeometry type covered completely** —
      and FR-81 stands at 99 of 837. The least-squares fit is in an internal `LeastSquares`
      helper, **not** inside `Plane`, because `Circle.ByBestFitThroughPoints` and
      `Line.ByBestFitThroughPoints` want the same arithmetic; whoever writes those should call
...
- [x] The ribbon's selection label is gone - `E8-T74`. It repeated the properties pane two inches away, and most of the time it read *Nothing selected*.
