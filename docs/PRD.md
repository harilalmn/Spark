...
**Owner:** Nicety
**Last updated:** 2026-09-13 (`Q16`: does Spark want a fabrication direction? FR-81 and FR-51 re-measured)
...
- **N5** — Not cross-platform *as a release*, **ever**. Windows only, permanently — **D16**
  hardened this from *for v1* to *for good*, so it is a statement about the product rather
  than about a version. Linux is currently still built and tested in CI, and no artefact is
  published for it; whether that job survives D16 is **Q15**. See **D14** and **D16**.
...
FR-56 in full, and FR-81, at 11.8%. M0 produced scaffolding, not behaviour; M1's first slice
...
| ID | Requirement | Status |
|---|---|---|
| FR-35 | `.spark` is plain, canonically formatted JSON — stable key order, invariant numbers — so graphs diff and merge in git. `.sparkz` zips a graph with its assets for sharing. | **Met** (E3-T20), for everything a graph has a file home for. The `.spark` format is met; the bundle landed 2026-09-11 as `SparkBundle` - the graph and its package folder, with a manifest and refusals for anything unsafe or unknown - and reaches the command line (`spark pack`, and a bundle accepted by `run`, `check` and `export`) and the application (File ▸ Open, File ▸ Share as bundle…). Assets, custom node definitions and a thumbnail join it when they have a file home. |
...
| FR-51 | `Mesh` with indexed vertices, tri and quad faces, optional normals, UVs and colours, and lazily built halfedge adjacency. Plus `PointCloud` and `GeometryGroup`. | **Met, with one type declined** (E2). `Mesh` and `MeshTopology` exist; `PointCloud` landed 2026-09-11 (`E2-T21`) - a kept bounding box and k-d-tree queries are what it has over a list. **`GeometryGroup` was declined**: a list already holds mixed geometry and a transform reaches every member through replication, so it would be a second spelling of a list. **What this row describes is the DATA STRUCTURE, and E2-T45 confirmed on 2026-09-13 that it says nothing about mesh OPERATIONS** - repair, remeshing, decimation and smoothing appear nowhere in it, in E2-T20 or in E2-T27, and Dynamo has six of them Spark does not (`E2-T68`). The row is met as written; the gap is that it was never written to cover them. |
...
| FR-57 | Geometry serialization: source-generated `System.Text.Json` with polymorphic discriminators and **per-type `schemaVersion`**, plus a compact binary `.sparkgeo` for bulk data. | **JSON done** (E2-T29, E2-T31): `GeometryJson`, every value self-describing with its own type and version, an unknown version refused rather than guessed at, and a reflection-driven test that fails on any geometry type without a round trip. Two departures, both recorded: it is **hand-written rather than source-generated**, and the binary `.sparkgeo` is still to come (E2-T30) |
...
| FR-60 | `Spark.Geometry.Planar`: `Point2d`/`Curve2d`, `Region`, and the Clipper2-backed boolean, offset and simplify pipeline, bridged by `Plane.To2d`/`To3d`. Not a peer 2D API. | **Partly met** (E2-T13). `Region`, the four booleans, offset and simplify landed 2026-09-11, through Clipper2 from one internal file; `Point2d` and the plane bridges already existed. **`Curve2d` waits for its first consumer** (`E2-T64`): nothing in the managed kernel reads a trim curve yet. |
| FR-81 | **Capability parity with Dynamo's geometry.** A person who knows Dynamo must never reach for a geometric capability in Spark and find it absent. Parity is of **capability**, not of type names, method names, parameter order, degenerate-case behaviour or tolerances — those are ours to choose, and **D8** removes any obligation to match them. The reference surface is `ProtoGeometry.dll` as installed with Revit 2026: **51 public types, 837 public members**. Progress is tracked member by member in [DYNAMO-COVERAGE.md](DYNAMO-COVERAGE.md) and held true by a two-way diff test against a checked-in manifest, so the register cannot drift from the code (E11-T23). | **Measured by the manifest since 2026-09-11 (E11-T23), and re-measured as each subsystem is assessed**: at 2026-09-13 the manifest stands at **210 `Done`, 68 `Planned`, 110 `Not planned`, 174 `Needs a decision` and 275 `Unassessed`**, which is 837. Four subsystems have been walked member by member against the assembly - §3.3 surfaces (E2-T42, 48 of 106), §3.4 solids (E2-T43, 24 of 55), §3.5 topology (E2-T44, 31 of 33) and §3.6 mesh (E2-T45, 41 of 65) - and every one was reading *0 reachable* or near it for capabilities that already worked. The hand count it supersedes follows. **99 of 837 reachable — 11.8%** (E2-T40 … E2-T46). 17.2% of the 575 members committed to, once §5's refusals and the undecided T-Spline surface are excluded. All 99 are in the value layer; there are no curves, surfaces, solids, meshes or topology counted here — **the curve layer exists but has not been walked against this register**, which is what E11-T23's two-way diff would stop happening. **Q11 is answered**: the parity promise moves exact solid booleans into 1.0, and they come from OpenCascade (**D2**, **D15**, [ADR-0020](adr/0020-occt-via-c-abi-shim.md)). **Q12 is still open**, and ADR-0020 does not touch it — OCCT has no subdivision modeller either |
...
| NFR-5 | `Spark.Geometry`'s published output contains **no native binaries**, asserted by CI. *Published* here means the `dotnet publish` directory, never nuget.org — nothing is packaged (**D11**). **Unchanged by ADR-0020.** The OCCT dependency lives in `Spark.Geometry.Occt`, a separate assembly; `Spark.Geometry` stays pure managed and independently distributable, and the CI assertion is untouched. | **Done** — `scripts/check-no-native-binaries.sh`, run on Windows and Linux (E1-T20). It publishes, then refuses a `runtimes/` directory, any file named like native code, and any `deps.json` declaring native or per-RID assets. **Proven to fire** by pointing it at `Spark.Desktop`, whose Avalonia dependency brings Skia and HarfBuzz natives |
...
| NFR-6 | Every change to the public surface of `Spark.Api` or `Spark.Geometry` is visible in a checked-in public-API baseline diff, and a breaking one is a recorded decision with a release note rather than a discovery. Adding is preferred to changing; the baselines are a **review aid, not a compatibility guarantee**. **ADR-0019**, which supersedes ADR-0009's strictly-additive rule. | **Done for the mechanism** — `Microsoft.CodeAnalysis.PublicApiAnalyzers [5.6.0]` is referenced from `Directory.Build.props` for all four contract projects, each with a `PublicAPI.Shipped.txt` and a `PublicAPI.Unshipped.txt`. `Spark.Geometry` declares 387 public members; the other three surfaces are empty. RS0016 is at error severity and **was proved to fire**, not assumed to. The *release note* half is answered as of `v0.1.0` (2026-09-02): the baselines are promoted at a release, so the next one's diff is the note |
...
| NFR-8 | **Restated on 2026-08-31, which is one of the two outcomes this row demanded, and it is not the suppression.** The tessellation of a closed solid is **geometrically closed and topologically split**: every kernel meshes a BRep face by face — ours and OCCT's alike — so every vertex on a shared edge exists twice, one copy per face, at identical coordinates. Nothing leaks; `IsClosed` counts *edges*, so a sound box reports twenty-four naked ones. **`Mesh.Welded(tolerance)` closes it, and is an operation rather than a default because a vertex carries one normal**: weld a cube's corners and it shades like a ball. So the guarantee is *a mesh that welds closed at a tolerance the caller chooses*, asserted against the provider's own output for a box, a cylinder, a union and a drilled plate. **Ask for it when the topology is what matters — a volume, an STL, a printer — and not when the shading is.** [NOTES.md N55](NOTES.md). *The original text follows.* Tessellation of a closed solid is watertight — a property-based test, not a spot check. **Caveat, and it is not a small one: under ADR-0021 the tessellation of a `Brep` happens behind the seam, so this property now tests a third party's mesher.** OCCT's mesher is not guaranteed watertight at default deflection. The requirement stands; what must be settled is whether it holds against OCCT's output at a deflection we choose, or whether it is restated to say precisely what it guarantees. **It must not quietly become a suppressed test.** Mesh tessellation stays ours and is unaffected. | **Answered (E13-T11)** |
...
| NFR-13 | No telemetry of any kind in v1. Opt-in crash reporting is considered post-1.0, with graphs excluded from any payload. | **Done, and no longer by construction alone — see [D22](#13-decision-log).** Nothing is collected and nothing is sent. Spark does make **one** outbound request on its own behalf, added 2026-09-02: an unauthenticated `GET` of its own GitHub release, once per session, to tell a user a newer version exists. It carries no identifier, no graph and no usage data, and it is switchable off and off by `--no-update-check` — but it is a request, and calling it nothing because its body is empty would be the dishonest way to keep this row green |
...
- **macOS and Linux release artefacts — permanently, not just for v1** (**D16**). Linux is
  currently still built and tested in CI and nothing is published for it; macOS is not built
  at all. This is now a closed question rather than a deferred one.
...
| R5 | **The node canvas collapses above ~1000 nodes**, which real graphs exceed | Would force a UI rewrite | Immediate-mode plus `SceneIndex` chosen precisely for this; M1.5 spike with 2000 synthetic nodes; LOD below 40% zoom; **benchmarked nightly since 2026-08-29** against a committed budget that states this risk as a number — 16.7 ms median is one frame at 60 fps ([ADR-0023](adr/0023-performance-budgets-not-a-benchmark-time-series.md)) |
...
| R15 | **MEASURED 2026-08-31, and the bracket was about the wrong thing.** The staged `win-x64` payload is **224.4 MB total**, of which OpenCascade is **52.0 MB — 23%**; the other 172.4 MB is the framework-dependent .NET publish, which predates ADR-0020 and which nobody had weighed either. The kernel is well inside this row's 40–160 MB bracket and nowhere near the 100 MB that would have reopened shipping it by default. **If the installer is ever too big, OpenCascade is not where to look first** ([NOTES.md N60](NOTES.md)). *The original entry follows.* **Native binary distribution.** A per-RID build matrix, an installer that grows, code signing for binaries we did not compile from our own source tree, and antivirus false positives on freshly signed unfamiliar native DLLs | Slips the release, and lands on the one person who does releases | Build once per RID as a cached artefact; ship OCCT as unmodified replaceable shared libraries, which the licence requires anyway. **The installer size is bracketed at 40–160 MB uncompressed and has not been measured** — M1.6 measures it, and until then the bracket is the honest statement. Excluding the Visualization module may remove FreeType; whether it does is open |
...
Eighteen decisions, each with the alternative that was rejected and why. Anything that
...
| **D2** | **Use an existing solid-modelling kernel rather than writing one.** Exact BRep booleans, trimming, filleting, sewing and STEP come from an engine we did not write; evaluation, values, meshes, planar geometry and every interchange writer stay ours. **Reverses this row's previous decision.** **Re-examined 2026-09-12 at the client's request — *can we implement our own solid geometry and completely avoid OCCT and Clipper2?* — and left standing.** Three things decided it. The **motive** was licensing and self-reliance, and `D25` settles the licensing half outright, so what remained was self-reliance alone. The **cost** is unchanged and is this project's own finding: exact surface-surface intersection with tangential and degenerate cases, tolerant topology, sewing, fillet and STEP are what `ADR-0002` called a multi-year research problem and what `DYNAMO-COVERAGE` §2 calls *what makes commercial kernels cost millions*. And the **shape of the code** makes the question reversible rather than urgent: `IBrepKernel` already abstracts all 21 operations behind `KernelResult<T>` and a capability set, so a kernel of our own would be a second provider and never a rewrite — which is precisely why it can wait for evidence instead of a decision now. **Clipper2 is not part of this question**: it is Boost-licensed and pure managed, carries no obligation at all, and stays until a consumer needs something it cannot do. [ADR-0020](adr/0020-occt-via-c-abi-shim.md) | Hold the previous staging and scope capability parity to *the end of 1.x* rather than *at 1.0* — which is what the PRD said, what R14 recommended, and what preserved every existing estimate | **The client was told the alternative and its price, and chose this one.** FR-81's instruction is that a Dynamo user must never reach for a geometric capability and find it absent, and [DYNAMO-COVERAGE §6.1](DYNAMO-COVERAGE.md#61-parity-on-solid-and-surface-commits-us-to-exact-solid-modelling) established that **70 members cannot exist without exact BRep booleans, trimming, filleting and sewing**. That is the work ADR-0002 staged last, R1 called research-grade, and §9 stated publicly as post-1.0. Deferring it to 1.x would have made the headline promise conditional on a research programme that might never land; writing it ourselves would have made 1.0 depend on it. Taking an engine is the only path that delivers the promise on a schedule anybody can plan against, and **it retires R1** — the largest risk in the project — rather than mitigating it again. **The cost must be read as two numbers, not one. Against the plan as written it is +7 to +11 weeks; against what was actually asked for it saves years.** Both are true, and the reason the first is positive is that **the plan as written never contained the expensive thing**: M6's 14 weeks bought mesh booleans, while exact booleans, fillet, chamfer and trim sat in the out-of-scope list. Parity was never funded, and this is what funding it looks like. |
...
| **D16** | **Windows is the only operating system Spark will ever support.** Not *for v1*, as **D14** had it — ever. No Linux or macOS release artefact, now or in any later version; the supported-platform statement in the README, the installer and the website says Windows, without a "for now" | Keep D14 as written, leaving cross-platform an open option that costs nothing to preserve and might be taken up in 1.x | **Client decision, and it is a scope decision rather than a technical one.** A single maintainer pays the signing, packaging, installer and support cost of every release target forever, and the AEC audience is on Windows; D14 already accepted that argument for v1 and the client has now accepted it permanently. **What it costs is the option, and the option was load-bearing in two places** — ADR-0020's choice of a C-ABI shim over C++/CLI was bought with it, and `M1.6-C1`/`M1.6-C2` test it on two operating systems. Both are reopened rather than resolved here, as **Q15**, because a decision that quietly rewrote an ADR while nobody was looking is the failure mode this log exists to prevent. **What it does not decide: whether the Linux CI job survives.** Supporting an OS is a release commitment; running a test job on one is a test technique, and D14 already held them apart. The Linux leg has found a real defect that had nothing to do with shipping on Linux ([NOTES.md N28](NOTES.md)), so its fate is Q15's, not this row's |
| **D17** | **The C-ABI shim stays, and D16 does not reopen it.** The OCCT binding remains a hand-written C ABI called through `LibraryImport`, and `M1.6-C1`/`C2`'s two-operating-system requirement is void: Windows alone satisfies them | Reverse to C++/CLI, now that the cross-platform option [ADR-0020](adr/0020-occt-via-c-abi-shim.md) paid a 15-25% premium for has been renounced by **D16** | **Answers Q15(a) and Q15(b).** The premium bought three things and D16 only devalues one. The other two are not about operating systems and are the reasons that still bind: **a small, deliberately chosen ABI surface** - a generated C++/CLI binding cannot reduce one, which is why Macad3D's runs to 170 files - and **surviving OCCT upgrades**, since a hand-written seam breaks where we chose it to and a generated one breaks wherever upstream moved a header. C++/CLI would also reverse **D7** and break the architecture test that forbids a `-windows` TFM, so the change is not confined to one project. **The moment to ask was now**, while `spark_occt` is unwritten; the answer is that nothing changes. **Q15(c) is separate and stays open**: whether the ubuntu CI job survives is a question about a *test technique*, not about a release commitment, and it has to be argued on its own merits |
| **D18** | **The ubuntu CI leg survives D16, and never builds the native provider.** It runs the managed build and test suite on every push; the OpenCascade shim is built on a Windows job alone, behind a cache keyed on `(occt-tag, vcpkg-baseline, shim-source-hash, rid)` | Drop the ubuntu leg entirely, now that **D16** has voided the rot-guard argument it was justified by; or keep it *and* build OpenCascade for `linux-x64` so the leg tests what Windows tests | **Answers Q15(c), and it answers it by separating two arguments that were being made as one.** The ubuntu leg was justified as a **rot-guard** — the thing that stopped cross-platform support decaying silently and kept ADR-0001's Avalonia investment real. **D16 voids that entirely**: there is no option left to keep alive. But the leg has independently caught a real defect ([NOTES.md N28](NOTES.md)), and *that* argument is a different one: a **second implementation of the same arithmetic**, with a different libc, a different floating-point library and a different culture default. It never depended on shipping a Linux build, and it is not affected by D16. **What D16 does change is the price of the expensive half.** Building OpenCascade for `linux-x64` in CI costs an hour per cache miss to guard a platform nobody ships, which is a cost with no remaining benefit. So the leg runs **managed-only** — and because `Spark.Geometry.Occt.Tests` skips itself when the shim is absent, it becomes a standing test of the **supported no-provider configuration** ([ADR-0021](adr/0021-brep-kernel-residency.md)) at no extra cost. **The failure this arrangement must not have is a green run that skipped everything**, so the Windows native job fails on a non-zero skip count |
| **D19** | **1.0 first; the end-user Help is completed after it.** The Help that does not yet exist — a topic per node family, the generated API reference, the in-product help renderer, a topic per `SPK####` code, and samples the build compiles — is written **after 1.0 ships**, not inside the milestones that build what it documents. **What is unchanged:** XML doc comments on the contract projects, the project documents, the ADRs and NOTES, all of which stay per-step | Keep **E10** at *M0 onwards*, writing each user topic in the same step that builds the feature — which is what the standing instruction in [AGENTS.md](../AGENTS.md) requires and what every milestone to date has done | **Client decision, and it is sequencing rather than scope: the Help is reordered, not descoped.** Four consequences, recorded because a sequencing decision that only gets remembered as *later* becomes *never*. **(1) It contradicts the standing instruction, so the instruction is amended rather than left to be quietly broken.** A rule every step violates is worse than no rule — this document already makes that argument about silent exemptions in `docs-freshness`. AGENTS.md now scopes *documented as end-user help topics* to the post-1.0 pass and holds the per-step obligation to XML docs and the project documents, which CS1591-as-error and the docs harness actually enforce. **(2) The cost is stated here rather than discovered at release.** 1.0 ships with nine concept topics, **no reference for 108 nodes**, no in-product help panel — so F1 does nothing — and `SparkDiagnostic.HelpTopicId`, a live seam carried by all **18** diagnostic codes, pointing at topics that do not exist. **(3) The verification rows move with it, and their order inverts — this is the part that is not free.** **E11-T2**, **E11-T4**, **E11-T5** and **E11-T6** were designed to be built beside a Help that grew continuously. Deferring makes a **bulk write** certain, and a bulk write is precisely the artefact `DocGenerator` was: 1,478 hand-maintained entries that drifted until 101 of 108 public constructors rendered blank. **The harness must therefore exist before the bulk write begins, not after it**, or this decision reproduces the exact failure E10 was designed to prevent. **(4) What the deferral genuinely buys**, and it is real: the topics get written once, against an API that has stopped moving, rather than repeatedly against one that has not. Two of the nine existing topics are already marked `Specification` rather than `Current` because they were written ahead of their code |
| **D20** | **`E12-T4`, the CAD-host proof, moves out of 1.0.** Spark ships standalone. The embedding *mechanism* stays in 1.0 and is built: `SparkSession` is a composition root with no UI in its reference graph, which `Spark.Architecture.Tests` enforces rather than asserts, and `HostThreadEvaluationScheduler` runs a whole graph on a foreign thread against a real work queue. **What defers is the demonstration inside a commercial CAD product**, and with it [Q5](#14-open-questions) | Keep `E12-T4` as a 1.0 acceptance criterion, and hold the release until somebody with a Revit or AutoCAD licence runs the proof | **Client decision, taken after asking why a CAD licence bore on a standalone application. It does not, and the register said so before this row did:** `E12`'s own scope note reads *M8 proves the seam, it does not ship a Revit plugin*, and the only mentions of either product in the entire source tree are doc comments explaining why `IEvaluationScheduler` exists. **Three things this is not.** It is not a descope of embedding: the seam, the scheduler and the UI-free host all ship, and an embedder can write the add-in shell against them. It is not a claim that embedding works — which is exactly why the claim must stop being made until somebody has run it. And it is not a saving: the work is deferred, not deleted. **What it costs, stated here rather than discovered later.** 1.0 ships with an embedding seam that has been tested against a *fake* host — one thread, one queue, a re-entrant marshal that throws — and never against Revit's external events or AutoCAD's document lock, which is where the surprises live. Until `E12-T4` is done, the honest public wording is **the engine is designed to be embedded and the seam is tested; it has not been run inside a CAD application**, and anything stronger than that is a claim nobody has earned. **`E12-T2` follows it out**, for the reason it was already open: an `IHostServices` designed without a host to try it against is speculation, and designing it *after* the first real add-in costs nothing that designing it now would save |
| **D21** | **`E12-T8`'s three words have three different answers, and only two of them are about licensing.** **Self-contained: no**, and **single-file: no** — both on the LGPL relink obligation, which needs OCCT's libraries to stay unmodified and replaceable, and a bundle that extracts to a temporary directory does not obviously preserve that. **ReadyToRun: no, and for a reason that is not legal at all** — it was measured and it does not pay | Ship ReadyToRun anyway on the general principle that precompilation helps startup, or leave the row open until counsel answers [Q13](#14-open-questions) item 2 | **Measured 2026-09-01 rather than reasoned.** ReadyToRun costs **+43.3 MB on the CLI (+105%)** and **+60.5 MB on the desktop (+35%, 172.7 to 233.2 MB)**, and buys, on seven and five runs respectively, **51 ms against 52 ms** for `spark --version` and **2,035 ms against 2,032 ms** for the desktop from launch to a rendered shell. That is nothing, twice. **What the measurement could not see, stated because it matters:** the desktop figure goes through the screenshot path, which waits for a full evaluation and then polls for a frame at 150 ms granularity, so **a JIT saving smaller than ~150 ms is invisible to it**. Even granting one, 60 MB on a 225 MB payload for a tenth of a second is a poor trade, and R15 already found the framework-dependent publish — not OpenCascade — to be where the payload actually is. **This is the reversible one of the three**: `PublishReadyToRun` is a single MSBuild property, so deciding no today costs nothing if a better measurement arrives. **Recording it separates a licence constraint from an engineering measurement**, which the row conflated: somebody reading it would have concluded all three were blocked on counsel, and only two are. *Nothing here is legal advice* |
| **D22** | **An update check is not telemetry, and saying so is not enough — NFR-13 has to be restated rather than reinterpreted.** The client asked for a label announcing a newer release. That needs one unauthenticated `GET` of Spark's own GitHub release, once per session, with an empty request body. **Nothing is collected**: no identifier, no machine, no graph, no usage. What it unavoidably reveals is what any HTTP request reveals — an IP address and a time, to GitHub, who already serve every download. So: it is **on by default**, because an update nobody hears about is an update nobody installs; it is **off in one click** from the Help menu, and off is remembered; `--no-update-check` refuses it for a session and cannot be overridden from the menu; the endpoint is **not configurable**, because a settable update endpoint is a mechanism for pointing an installation at a build that is not Spark; and **every failure is silence**, so a user offline sees exactly what a user on the newest build sees. | Ship it silently as an implementation detail on the grounds that NFR-13 says *telemetry* and this is not telemetry; or refuse the request and let users find out about releases by visiting the repository | **Neither.** The first is how a no-telemetry promise stops being worth anything — the next request is always also "not really telemetry". The second declines a reasonable request over a distinction the user never asked us to draw. The row above now says what Spark does, in the place somebody would go to check |
| **D22a** | **The badge opens a browser; it does not download or install.** | Build an in-application updater that fetches and runs the installer | **Not while nothing is signed.** An unsigned binary fetching and executing another unsigned binary is a supply-chain shape Spark should not teach its users to accept. The release page shows the notes, the attachments and the checksums, and the user decides. Revisit when `E13-T17`'s signing half exists |
| **D23** | **A `.spark` file's text is written as typed, and the one-time diff is accepted** (`E3-T24`, [ADR-0026](adr/0026-spark-file-text-is-written-as-typed.md)). `SparkFile` writes with the relaxed encoder, so `+`, quotation marks, `<`, `>`, `&`, `'` and non-ASCII read in a diff as they were typed. An older file reads back to the same document and takes the new spelling the first time it is saved. | Write relaxed only for text that changed, so an older file never shows a spelling diff | The reader decodes every string, so the writer cannot know an old spelling without carrying it through `GraphDocument` - new state on a contract type for a cosmetic property - and a file would then hold two spellings of one character for good, changing spelling as well as content on the day anyone edited that string. `E7-T7`'s byte-for-byte promise is about a file this build wrote, and it holds. |
| **D24** | **A first-party node method may gain a trailing optional `CancellationToken`, and that is a minor change, not a major one** (`E3-T12`). `Spark.Nodes.Core.Surface.ToMesh` gained one: the node's key, its ports and every saved graph are unchanged, and C# source that calls the method still compiles. | (a) Add an overload instead, as AGENTS.md prefers; (b) an ambient token the node reads from a context; (c) leave first-party nodes unwired, or (d) count the change as major, as the release rule for a changed shipped member says | (a) An overload of a node method is a **second node**: both would carry the same ports, so the importer would disambiguate them by port name and **change the existing node's key**, breaking every graph that uses it - the opposite of additive. (b) Hidden state on the evaluation path, which this codebase refuses for tolerance and should not start for tokens. (c) Leaves the one node that can take a long time unstoppable. (d) The rule protects compiled callers of a published library; `Spark.Nodes.Core` is not published (ADR-0019) and its node methods are reached through the importer, which sees no difference. The baseline records it with the repository's first `*REMOVED*` line, so the change is a visible diff rather than a quiet one. |
| **D25** | **Spark is released open source, and that settles the OpenCascade licensing questions.** The LGPL duty attaches to distributing OCCT's binaries, not to publishing our source — but the question with teeth was whether the shim is a derivative work under §5, because that answer would have reached into code we own. With the source public, both answers lead to the same place | Keep the source closed and pay counsel for `Q13` items 1 and 3 before M6, carrying the uncertainty until then | **Client decision, 2026-09-12**, taken after asking what the licensing concern even was for a closed-source product — a fair question, and the answer is that it was never about our source but about the shim's status. Everything the licence asks is already done: dynamic linking, unmodified and replaceable libraries, no single-file or AOT seal (`D21`), the LGPL and exception texts shipped, notice in the About box, README, installer and release notes, and a source offer through the pinned tag and vcpkg baseline. **A second consequence, and it is why two workflows were made dormant rather than deleted**: a public repository's Actions minutes are free, so the CI rows blocked by `E13-T19` unblock themselves at release. **Nothing in this document is legal advice** |
| **D26** | **Nothing is signed, and no code-signing certificate is bought.** The installer ships unsigned, and the first run shows SmartScreen's *Windows protected your PC* until a user chooses *More info* then *Run anyway* | Buy an OV or EV certificate, store it on a token or in a cloud signing service, and add a signing step to the local release | **Client decision, 2026-09-12**: *Forget it. We are not buying any certificates.* The consequence is recorded rather than discovered: `E12-T9` is withdrawn and `E13-T17` closes with it. Three things already soften it and none needs a certificate — the install is per-user and **unelevated**, so Windows never asks for administrator over an unsigned binary; a SHA-256 is published beside every artefact; and the release notes say plainly that nothing is signed. **A self-signed certificate is not a cheaper version of this**: SmartScreen's reputation follows the issuer, so it buys nothing at all |
...
| Q5 | **Deferred with `E12-T4` by [D20](#13-decision-log), not answered.** Is Revit or AutoCAD the host that proves `Spark.Host`? The host-thread scheduler is the same either way and is built; the add-in shell, licensing and test loop are not. **It stops gating 1.0** — and note that this row was always the first obstacle to `E12-T4`, which could not begin without an answer to it. | **After 1.0**, with `E12-T4` |
...
| Q13 | **Reduced 2026-09-12 by D25 and no longer a question for counsel.** Spark is released open source, and the sharp question — whether `spark_occt` is a *work that uses the Library* or a derivative under §5 — stops having different answers: under either reading the source is public and a user can relink. What remains is housekeeping this project already does: dynamic linking, unmodified replaceable libraries, the LGPL and exception texts, notice in four places, and the source offer through the pinned tag and recorded vcpkg baseline. Two items are worth doing anyway because they cost a publish step each — **publish the shim's own source** with the binaries, and **attach an OCCT source archive** to a release rather than relying on a tag reference. **Nothing in this document is legal advice.** *The original question follows.* **The six licensing questions for counsel.** The central one: is a thin shim whose entire purpose is to expose OCCT a *work that uses the Library* under the Open CASCADE exception, or a derivative work under LGPL §5? Then: whether single-file, trimmed or AOT publishing is compatible with the relink obligation; whether vcpkg's port declaring `LGPL-2.1-only` — **omitting the exception** — creates exposure; what *prominent notice in supporting documentation* requires concretely; what obligations attach to a user embedding `Spark.Host` in a commercial add-in (**D5**); and whether the source offer is satisfied by a tag reference or needs a hosted archive. **Nothing in this document is legal advice**, and this row exists because it is not. The full list is in [ADR-0020](adr/0020-occt-via-c-abi-shim.md). | **Items 1 and 3 before M6.** The rest before 1.0 |
| Q14 | **Answered on evidence, 2026-08-31 — see E13-T14 and [NOTES.md N56](NOTES.md).** **Distinct shapes may be operated on concurrently; one shape must not be touched by two threads at once.** Twenty threads x twenty-five union-and-tessellate gave 500 results in 2.73 s with zero failures and every volume right, and the shim's error state is thread-local and was checked rather than assumed. **The conservative single-writer fallback this row named is not needed for the case replication actually produces**, which is the case it would have cost the most. *The original question follows.* **What is OCCT's real thread-safety envelope?** May the parallel evaluator call the shim concurrently, and at what granularity? Documented guidance is thin, and **R20** is a top-three risk that cannot be mitigated until this is known. *How to find out:* read the upstream source of the specific packages we call, and stress the shim at the evaluator's real thread count. The conservative fallback is a single-writer policy, which would cost throughput on exactly the workload replication makes common. | Answered at M6, by stressing rather than by reading |
...
| **Q15** | **What does D16 reopen?** Two things, and they were bought with the cross-platform option D16 has just given up. **(a) The binding.** [ADR-0020](adr/0020-occt-via-c-abi-shim.md) chose a hand-written C-ABI shim over C++/CLI at a stated **15–25% effort premium**, and the payoff it names is *buying back the entire cross-platform option*. Under D16 that payoff is worth nothing, and C++/CLI has the better debugging story and is proven at application scale by Macad3D. **The honest counter, which must not be left out of the argument:** the shim was also chosen for a **small, deliberately chosen ABI surface** and for surviving OCCT upgrades, and neither of those reasons is about operating systems — a generated C++/CLI binding cannot reduce the surface, which is why Macad3D's runs to 170 files. C++/CLI would additionally reverse **D7** and break the `-windows`-free architecture test. **The moment to ask is now**: `spark_occt` is not written yet, so reopening is cheap today and expensive at M6. **(b) The two-OS criteria.** `M1.6-C1` and `M1.6-C2` require the OCCT build and one boolean on Windows **and** Linux. If the Linux CI job goes, so does the reason; if it stays, the Linux legs stay with it. **This is the item blocking M1.6 today** — the Windows toolchain exists and was verified by compiling, and only the Linux leg is missing. **(c) follows from both: does the ubuntu CI job survive at all?** Its rot-guard justification is void under D16, but it has independently caught a real defect ([NOTES.md N28](NOTES.md)) and it is where floating-point and culture-dependent differences surface. That is a different argument for keeping it, and it has to be made on its own or the job has to go | **(a) before `E13-T2` and therefore before M6; (b) and (c) before M1.6 is taken**, which is next after M4 |
| **Q16** | **Does Spark want a fabrication direction at all?** `Mesh.MakeHollow` and `Mesh.GenerateSupport` are ProtoGeometry's two 3D-printing members, found by `E2-T45`'s assessment of [DYNAMO-COVERAGE §3.6](DYNAMO-COVERAGE.md#36-mesh--2-types-65-members-41-reachable). They are a **product direction rather than a kernel primitive**, and the question is not whether they are useful — hollowing a model and generating supports are what a printing user does — but whether the kernel is the thing that owns them. **The recommendation is no, on §5 [d]'s argument**: a hollow is a wall thickness and an escape hole, a support is a scaffold generator with its own taxonomy of tree, lattice and pillar, and both are built over the mesh API rather than under it. Panelling lost the same argument and became a node-package candidate. **What makes this a question rather than a decision is that FR-81 promises capability parity**, and two members refused on taste are two members a Dynamo user reaches for and does not find — so if the answer is no it belongs in §5 with a reason, not left unassessed. Note `IBrepKernel.Shell` already hollows a **solid** to a wall thickness and is a different operation on a different representation; the names collide and the capabilities do not | Before M5 closes |
