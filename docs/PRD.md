...
| FR-25 | Node and port descriptions come from the assembly's sidecar XML documentation file, so any library shipping its `.xml` gets tooltips with no extra work. | **Done** (E5-T7) — `summary` for the node, and **`param` and `returns` for its ports**. The port half was missing until a canvas tooltip was built and had nothing to show ([N29](NOTES.md)) |
...
| R5 | **The node canvas collapses above ~1000 nodes**, which real graphs exceed | Would force a UI rewrite | Immediate-mode plus `SceneIndex` chosen precisely for this; M1.5 spike with 2000 synthetic nodes; LOD below 40% zoom. **The nightly now runs it** — but records rather than gates it, because a runner has no GPU and the spread of a software-rendered frame time on that hardware is not yet known (`E1-T34`, [ADR-0023](adr/0023-benchmarks-gate-allocation-not-time.md)). The `SceneIndex` half *is* gated, on allocation. **The spike figures this row rests on were measured over a 120-frame window while the run reported 500** ([N31](NOTES.md)); the margin was three orders of magnitude, so the conclusion holds and the numbers need re-quoting |
...
Sixteen decisions, each with the alternative that was rejected and why. Anything that
...
| **D16** | **The nightly benchmark gates bytes allocated per operation, and gates nothing else.** Ceilings live in `bench/baseline.json`; timings are recorded and never enforced; a benchmark with no baseline entry fails the job. [ADR-0023](adr/0023-benchmarks-gate-allocation-not-time.md) | A generous timing threshold, or committing the results to the repository as a git time series — **which is what the task as written asked for** | Allocation is a property of the code and not of the host, so the same build allocates the same bytes on a laptop and on a shared runner; it is the one figure a noisy datacentre VM cannot move. A timing threshold was rejected on measured arithmetic rather than on principle: four consecutive runs on a **quiet** machine spread the canvas median 20%, so any honest threshold would have to sit wide enough to miss most real regressions while still going red for host weather — and a gate that fails for reasons nobody can act on teaches people to override gates generally. The committed time series was rejected on price rather than on value: it needs `contents: write` on a scheduled job and puts an automated writer on `main` permanently, where 90-day artifacts carry the same numbers and a human-updated baseline carries the part that must be durable. **The accepted cost is that slow drift is invisible** — 4% a night forever stays under a 5% ceiling. |
