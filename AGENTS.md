...
- `dotnet test Spark.slnx` — **1,179 tests across eight projects**, all passing. The same
  1,179 are reported by `scripts/run-tests.sh`, which runs each project as the executable
  ...
  (321) covers the graph, the replicator, the importer, the cache, the schedulers and the `.spark` format, including a two-way diff against the
