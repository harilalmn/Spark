...
public enum NodeMatch
{
    ...
    /// <summary>The query is the node's capitals: <c>cfcr</c> for <c>Circle.FromCenterRadius</c>.</summary>
    CamelHump = 4,
...
/// <param name="DisplayName">The node's name, such as <c>Circle.FromCenterRadius</c>.</param>
...
/// is not a matter of taste. What it buys is that <c>cfcr</c> finds <c>Circle.FromCenterRadius</c>,
...
public static class NodeSearch
{
        ...
        string displayName, string? category, string? description, string? query)
    {
        ...
        // The part after the last dot is what a user usually means. Circle.FromCenterRadius is found
        // by "circle" and by "bycenter", and both should feel like a prefix match.
    ...
    /// The tie-break used to be name length, which put <c>Circle.FromCenterRadius</c> above
    /// <c>Circle.FromCenterNormalRadius</c> — a rule that is defensible and that nobody reading the
    ...
    /// </summary>
    /// <remarks>
    /// <c>Circle.FromCenterRadius</c> reduces to <c>CBCR</c>. A digit counts as a hump because
